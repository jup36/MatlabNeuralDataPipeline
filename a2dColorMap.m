...
numPoints = 1000;
amplitude = 3000;
xx = amplitude * rand(1, numPoints);
yy = amplitude * rand(1, numPoints);
subplot(2, 1, 2);
grid on;
thisColor = zeros(numPoints, 3);
for k = 1 : numPoints
	col = ceil(xx(k) * numColors / amplitude);
	row = ceil(yy(k) * numColors / amplitude);
	thisColor(k, :) = [colormap2D(row, col, 1), colormap2D(row, col, 2), colormap2D(row, col, 3)];
	fprintf('(xx,yy) = (%6.1f, %6.1f), row = %3d, col = %3d, thisColor = (%.4f, %.4f, %.4f)\n', xx(k), yy(k), row, col, thisColor(k, :));
% 	plot(xx(k), yy(k), '.', 'Color', thisColor, 'MarkerSize', 30);
% 	hold on;
end
scatter(xx, yy, 30 * ones(1, numPoints), thisColor, 'filled');
axis('square');
grid on;
xlim([0, amplitude]);