...
for f = 1:length(filePaths)
    ...
    [~, rrrRezR2] = stimEffect_neuralTrj_cv_stim_pstim_cg(filePath{1}, dims, folds); % args: dimensions and folds
...
save(fullfile('/Volumes/Extreme SSD/js2p0/collectData', ['stimEffect_neuralTrj_rrr_Cg_stimPstimPrepExtWoTo', sprintf('_Dims%d', dims), sprintf('_Folds%d', folds)]), 'mR2*', 'medR2*')
...
save(fullfile('/Volumes/Extreme SSD/js2p0/collectData', ['stimEffect_neuralTrj_rrr_Cg_stimPstimPrepExtWoTo', sprintf('_Dims%d', dims), sprintf('_Folds%d', folds)]), 'stat', '-append')

%% plot
fig = scatter_row_by_row(mR2_tbyt_noStim_pStim_stim, [0, 0.4]); 
print(fig, fullfile('/Volumes/Extreme SSD/js2p0/collectData/collectFigure', ...
    ['stimEffect_neuralTrj_cv_collect_RRR_rez_mR2', sprintf('_Dims%d', dims), sprintf('_Folds%d', folds)]), ...
    '-vector', '-dpdf');
