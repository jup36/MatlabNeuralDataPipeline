...
for f = 1:length(filePath)
    ...
    if isfield(jkvt, 'stimLaserOn')
        ...
        % reach angle
        % median subtraction
        rchAngC = {ss.rchAngDeg};
        rchAngMed = nanmedian(cell2mat(rchAngC));
        rchAngMedSubC = cellfun(@(x) x - rchAngMed, rchAngC, 'UniformOutput', false);
        assert(length(rchAngC)==length(rchAngMedSubC));
        [ss.rchAngDegMedSub] = rchAngMedSubC{:};
        ...
        rez.rchAngMedSub{f} = [ss(trI.spNoStimI{f}).rchAngDegMedSub]; % RA: all
        ...
        rez.rsAlignRchAngMedSub{f} = [ss(trI.spNoStimRsAlignI{f}).rchAngDegMedSub];
        ...
        rez.rchAngStimBtMedSub{f} = [ss(trI.stimBtId{f}).rchAngDegMedSub]; % RA: breakthrough trials
        ...
        if isfield(trI, 'stimFullBtI') && any(trI.stimFullBtI{f}==1)
            ...
            rez.rchAngStimFullBtMedSub{f} = [ss(trI.stimFullBtI{f}==1).rchAngDegMedSub]; % abs RA: breakthrough trials
        ...
        if isfield(trI, 'stimFullBtI') && any(trI.stimFullBtI{f}==1) && isfield(ss, 'hVelB_fullBt')
            rez.maxForceStimFullBt{f} = cell2mat(cellfun(@(a) nanmax(abs(a)), rez.forceBstimFullBt{f}, 'UniformOutput', false));  % max speed breakthrough with full inactivation
            rez.meanForceStimFullBt{f} = cell2mat(cellfun(@(a) nanmean(abs(a)), rez.forceBstimFullBt{f}, 'UniformOutput', false));  % mean speed breakthrough with full inactivation
        end
    end
...
rezCol.controlRsAlignRchAngRawMedSub = cell2mat(rez.rsAlignRchAngMedSub);
rezCol.controlRsAlignRchAngRawMedSub(rezCol.controlRsAlignRchAngRawMedSub==0) = NaN;
...
rezCol.stimBtRchAngRawMedSub = cell2mat(rez.rchAngStimBtMedSub);
rezCol.stimBtRchAngRawMedSub(rezCol.stimBtRchAngRawMedSub==0) = NaN;
...
rezCol.stimFullBtRchAngRawMedSub = cell2mat(rez.rchAngStimFullBtMedSub);
rezCol.stimFullBtRchAngRawMedSub(rezCol.stimFullBtRchAngRawMedSub==0) = NaN;
