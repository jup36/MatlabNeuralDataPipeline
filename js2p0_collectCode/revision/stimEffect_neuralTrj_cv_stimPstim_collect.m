...
for f = 1:length(filePaths)
    filePath = GrabFiles_sort_trials('js2p0_tbytSpkHandJsTrjBin_50ms_stimPstimPrepExtWoTo_', 0, filePaths(f));
    %filePath = GrabFiles_sort_trials('js2p0_tbytSpkHandJsTrjBin_50ms_stimPstimWoTo_', 0, filePaths(f));
    [~, rrrRezR2] = stimEffect_neuralTrj_cv_stim_pstim(filePath{1}, 20, 10); % args: dimensions and folds
...
save(fullfile('/Volumes/Extreme SSD/js2p0/collectData', ['stimEffect_neuralTrj_rrr_stimPstimWoTo', sprintf('_Dims%d', dims), sprintf('_Folds%d', folds)]), 'mR2*', 'medR2*')

%% Stats
% prepare session-by-epoch data
mR2_noStim_pStim_stim = [cell2mat(mR2.noStim)', cell2mat(mR2.pStim)', cell2mat(mR2.stim)']; 
mR2_tbyt_noStim_pStim_stim = [cell2mat(mR2.noStim_tbyt)', cell2mat(mR2.pStim_tbyt)', cell2mat(mR2.stim_tbyt)']; 
medR2_noStim_pStim_stim = [cell2mat(medR2.noStim)', cell2mat(medR2.pStim)', cell2mat(medR2.stim)']; 
medR2_tbyt_noStim_pStim_stim = [cell2mat(medR2.noStim_tbyt)', cell2mat(medR2.pStim_tbyt)', cell2mat(medR2.stim_tbyt)']; 
...
save(fullfile('/Volumes/Extreme SSD/js2p0/collectData', ['stimEffect_neuralTrj_rrr_stimPstimWoTo', sprintf('_Dims%d', dims), sprintf('_Folds%d', folds)]), 'stat', '-append')

%% plot
fig = scatter_row_by_row([cell2mat(mR2.pStim)', cell2mat(mR2.noStim)', cell2mat(mR2.stim)'], [0, 0.4]); 
