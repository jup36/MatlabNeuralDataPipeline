...
for f = 1:length(filePath)
    ...
    dPRM_Dir = dir('**/*glm_dPrime_Cg_vec_norm_WR*.mat');
    
    if ~isempty(spkDir_Cg) && ~isempty(dPRM_Dir)
        wr = strfind(filePath{f}, 'WR');
        saveName = filePath{f}(wr:wr+10);
        %dPrmDir = dir(fullfile(filePath{f}, strcat('glm_dPrime_*')));
        load(fullfile(dPRM_Dir.folder, dPRM_Dir.name),'dPrmC_Cg')
        ...
        for i_cell = 1:length(dPrmC_Cg)
            thisCellId = strcat(saveName,sprintf('_Cell#%d',i_cell));
            if ~isempty(dPrmC_Cg{i_cell})
                counter = counter+1;
                s = dPrmC_Cg{i_cell}; % structure for this cell
...
%print(fullfile('D:\Junchol_Data\JS2p0\collectData\collectFigure','dPrime_Cg_vec_norm'),'-dpdf','-painters')
%save(fullfile('D:\Junchol_Data\JS2p0\collectData','dPrime_Cg_vec_norm_collectRez'),'dPrmRez')
%load(fullfile('D:\Junchol_Data\JS2p0\collectData','dPrime_Cg_vec_norm_collectRez'),'dPrmRez')
...
for tp = 1:5:100
    ...
    for cc = 1:length(dPrmRezCtx)
        ...
        if dPrmRezCtx(cc).s.trjDistZero_sigI(tp)==1
            scatter(currPt(1),currPt(2),50, 'MarkerEdgeColor', 'none', 'MarkerFaceColor', thisColor, 'LineWidth', .5,  'MarkerFaceAlpha',.5)
        else
            scatter(currPt(1),currPt(2),25, 'MarkerEdgeColor', 'none', 'MarkerFaceColor', [.25 .25 .25], 'LineWidth', .5,  'MarkerFaceAlpha',.5)
        end
    end
    hText1 = text(0.90,-1.75,strcat(sprintf('%d',timeX(tp)),'ms'),'FontSize',18);
    print(fullfile('D:\Junchol_Data\JS2p0\collectData\collectFigure\dPrm_snapShot',strcat('dPrime_Cg_scatter_snapShots_',sprintf('%d',timeX(tp)),'ms')),'-dpdf','-painters')
    close all
end
hold off;
...
hold off
%print(fullfile('D:\Junchol_Data\JS2p0\collectData\collectFigure','dPrime_vec_norm_Cg_proportionPie'),'-dpdf','-painters')
dPrmType_Cg_distribution = cell2mat(cellfun(@(a) size(a,1), dPrmTtC, 'un', 0))./ttNumbCell;
