...
for f = 1:length(filePath)
    preprocessingForDecoder_KalmanFilterHTrjF_reachPull_withCg(fullfile(filePath{f}, 'Matfiles'))
    fprintf('processed file #%d\n', f) 
end
 
%% Run KF decoder
for f = 1:length(filePath)
    hTrjDecodingKalmanFilter_hTrjF_reach_pull_PosVelXYZ_params_new(fullfile(filePath{f}, 'Matfiles'), filePath{f}(end-10:end))
    fprintf('processed file #%d\n', f) 
end
 
%% Run KF decoder
for f = [1:6, 8:15]
    hTrjDecodingKF_hTrjF_reach_pull_PosXYZ_weight_crossing(fullfile(filePath{f}, 'Matfiles'))
    fprintf('processed file #%d\n', f) 
end