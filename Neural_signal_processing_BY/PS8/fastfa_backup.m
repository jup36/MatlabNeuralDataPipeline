...
% assignopts(who, varargin);
  ...
  L     = randn(xDim,zDim)*sqrt(scale/zDim); % init. factor loadings (xDim x zDim)
  Ph    = diag(cX); % diagonal of uniqueness matrix (xDim x 1)
  ...
  for i = 1:cyc     % maximum number of EM iterations
    ...
    % =======
    invC = inv(L*L' + diag(Ph)); % invC = inv(Sigma), Sigma equals covariance of X (Xdim x Xdim) 
    ...
    % M-step
    % =======
    L  = cX_beta / EZZ;     % update loading matrix(vector) using Equation 12 in the note 
    Ph = diag(cX) - sum(cX_beta .* L, 2);   % Sigma (observation noise), Equation 13
