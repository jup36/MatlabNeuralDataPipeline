function allSpike_distJP( varargin )
...
prompt = 'What is the probe insertion depth? ';
dv = input(prompt); 
%dvCosConvert   = cos(10/180*pi);
%brainDepth = dv.*dvCosConvert; 
...
dataType = res.dataType; % 0 for neuropixels 1.0, 1 for NP 2.0. 
...
depthX = 1:nRow;
estZ = dv-res.zPos(depthX); 
imagesc(xVal(1:xMax), -estZ, cdf(:,(1:xMax)));
...
caxis([.5 20]);
fileNameMarker = strfind(fileName,'_'); 
print(fullfile(fileDir,strcat('spikeAmpHistInverseCdf_',fileName(1:fileNameMarker(2)-1))),'-dpdf','-painters')

f3 = figure('Units','Centimeters','Position',[8,5,14,18]);
subplot(3,1,1);
plot(-estZ(goodChanList+1), MAD_goodChan(zsorder));
...
subplot(3,1,2);
plot(-estZ(goodChanList+1), currRate(zsorder));
...
subplot(3,1,3);
plot(-estZ(goodChanList+1), curr90th(zsorder), -estZ(goodChanList+1), curr99th(zsorder));
...
print(fullfile(fileDir,strcat('MAD_eRate_90-99uV',fileName(1:fileNameMarker(2)-1))),'-dpdf','-painters')


if isfield(res, 'probeType' )
    plotChanFilesJP( res.probeType, estZ, {fullfile(fileDir,chanOutName)} );
end
