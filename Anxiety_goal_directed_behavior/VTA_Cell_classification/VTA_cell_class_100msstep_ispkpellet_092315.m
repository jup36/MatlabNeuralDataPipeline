...
saveName = 'SUA_AGB_VTA_cellclass_100ms_step_ispkpellet_092315';      

%% set user variables!
uv.step = 0.1;     % step in sec (50 ms)
...
%% ROC analysis
cd('C:\Users\jup36\Desktop\Data\Anxiety_goal_directed_behavior\MAT\PeriISPKPellet_extended_RasterData_VTA')    % directory for the raster data, binned raster data
...
ROCmat = zeros(uv.numbunit,3000/100);    % 3000 ms window binned in 100 ms

for u = 1:uv.numbunit   % # of units
    
    load(FileList(u).name,'binned_raster_data','raster_labels');    % load the current unit's binned data
    
    base100ms = sum(full(base.SC{u}(1:50,1:40)),2)/(2*10);  % 2 for 2 s 10 for 100 ms (100 ms bin was used for the peri-pellet binned spike counts
    base100ms(:,2) = 0;
       
    for t = 1:size(binned_raster_data,2)    % # of timebins
         
        temp100ms = binned_raster_data(1:50,t);
        temp100ms(:,2) = 1;
...
InitParams.mu = zeros(3,2);      % Initialization of the cluster center features(3) x clusters(3) 
...
InitParams.pi = [2/3, 1/3];      % Initialization of the prior cluster probability 

% cluster1
InitParams.mu(1,1) = 0;  % feature 1: amp
InitParams.mu(2,1) = 0;    % feature 2: halfwidth
InitParams.mu(3,1) = 0.1;    % feature 3: avFR
...
% % cluster3
% InitParams.mu(1,3) = -1.2;   % feature 1
% InitParams.mu(2,3) = 0;      % feature 2
% InitParams.mu(3,3) = 0;      % feature 3
% 
% InitParams.Sigma(:,:,3) = diag(diag(cov(pc123)));   % Assume that all cluster covariances are initialized to the same covariance matrix 
...
%     elseif EM.c(1,u) == 2 % class 3
%     scatter3(pc123(u,1),pc123(u,2),pc123(u,3),'MarkerFaceColor',[117 172 66]./255,'MarkerEdgeColor',[117 172 66]./255)  % scatter for class2 units
    elseif EM.c(1,u) == 2 % class 3
...
cd(saveDirectory)
save(saveName,'ROCmat','dat','pellet','EM')
