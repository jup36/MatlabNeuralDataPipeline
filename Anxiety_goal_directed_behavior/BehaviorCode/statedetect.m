function [finalstatebegin, finalstateshift, finalstatelatency, finalimmobile, finalimmobilecount, finalimmobileITI, finalimmobilecountITI] = statedetect( indexstate, state, stateITI, entirestate, time, timeconstant, motion )
...
switch isempty(itiarray)
    ...
    case 0      % valid trials proceed to the following script
        ...
        if isempty(indexstatearray) || isempty(itiarray) || isempty(statebegin) == 1        % return NaN if there is no trial in this block
            ...
            if (entirestate(end,1) == state && entirestate(end-1,1) == state) == 1      % this occurs when the session has been terminated within the state, get rid of the last two elements in the statebegin array.  
