...
Xprm1 = X*rez1(i_cell).prm1_norm;  % when fitting the neural FRs make sure to use normalized weights (prm1_norm) because that's what is optimized
%figure; plot(Xprm1);
expXprm1 = basis.normal_filter(exp(Xprm1), p.filter_sigma, 10);
spike_bin_i_cell = histcounts(rez1(i_cell).spike_time, tV.time_bin);
...
for i_cell = 1:length(rez)
    ...
    model_rStart_psth2s = func.alignGlmOutToTaskEvent(tV.evtRstart, time_bin, expXprm1, 10, 2000); % fitted rate aligned to reachStart
    spike_rStart_psth2s = func.alignGlmOutToTaskEvent(tV.evtRstart, time_bin, spike_bin_conv_i_cell, 10, 2000); % fitted rate aligned to reachStart
