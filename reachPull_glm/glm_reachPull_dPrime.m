...
for i_cell = 1:size(spkTimesCell,2) 
    ...
    if spike_rate>=0.5
        ...
        % d' trial types
        dPrmC{i_cell,1} = dPrime_trialType(spike_rStart_conv, [tV.leloI(pStartI), tV.lehiI(pStartI), tV.riloI(pStartI), tV.rihiI(pStartI)], [-1 -1 1 1; -1 1 -1 1]); 
...
rs_dPrm_2d_X = sum(cell2mat(reshape(cellfun(@(a) squeeze(a(1,:,:)), rs_dPrm_2d, 'un', 0),1,1,[])),3); % X-dim (left-right)
rs_dPrm_2d_Y = sum(cell2mat(reshape(cellfun(@(a) squeeze(a(2,:,:)), rs_dPrm_2d, 'un', 0),1,1,[])),3); % Y-dim (low-high)

rs_dPrm_2d_XY = [reshape(rs_dPrm_2d_X,[],1),reshape(rs_dPrm_2d_Y,[],1)]; 

% resampled bivariate distribution mean, cov to get the pdf 
mu_rs_dPrm_2d_XY = nanmean(rs_dPrm_2d_XY,1); 
cov_rs_dPrm_2d_XY = cov(rs_dPrm_2d_XY); 

% cdf
[Xs,Ys] = meshgrid(linspace(-3,3,600)',linspace(-3,3,600)'); 
XYs = [Xs(:),Ys(:)]; 
p_cdf0 = mvncdf(XYs,mu_rs_dPrm_2d_XY,cov_rs_dPrm_2d_XY); 
...
for tt = 1:size(ttI,2) % trial Types (e.g. lelo, lehi, rilo, rihi)
    ...
    dPrm_1d{tt} = (nanmean(tmpMat,2)-nanmean(tmpNoMat,2))./(nanstd(tmpMat,[],2)+nanstd(tmpNoMat,[],2)); 
...
for j = 1:size(dPrmRez.trj2d,1)
    [~,minDistI] = min(dist2d(dPrmRez.trj2d(j,:)-XYs)); 
