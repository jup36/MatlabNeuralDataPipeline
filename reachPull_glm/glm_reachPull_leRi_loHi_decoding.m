...
for f = 1:5 % 2-fold cross-validation for decoding
    ...
    for i_cell = 1:size(spkTimesCell,2)%1:size(spkTimesCell,2)
        ...
        if spike_rate>=0.5
            ...
            spike_bin_conv_cnt = basis.normal_filter(spike_bin', p.filter_sigma, p.DT); % p.filter_sigma = 100 ms
            
            %figure; hold on; plot(spike_bin_conv); plot(expXprm1); hold off;
            rez(i_cell).r2_all{f} = max(r2(spike_bin_conv, expXprm1),0);
            % calculate r^2 within PSTHs
            allTrainThisFold = cell2mat(train_mv_pStartI(:,f)); 
            model_rStart_psth2s = func.alignGlmOutToTaskEvent(tV.evtRstart(allTrainThisFold), time_bin, expXprm1, 10, 2000); % fitted rate aligned to reachStart
            spike_rStart_psth2s = func.alignGlmOutToTaskEvent(tV.evtRstart(allTrainThisFold), time_bin, spike_bin_conv, 10, 2000); % fitted rate aligned to reachStart
            rez(i_cell).r2_psth{f} = max(r2(reshape(spike_rStart_psth2s,[],1),reshape(model_rStart_psth2s,[],1)),0);
            
            %% grab results specific to each cell
            rez(i_cell).prm1_norm{f} = prm1_norm;
            rez(i_cell).prm1{f} = prm1;
            rez(i_cell).prm0{f} = prm0;
            rez(i_cell).spike_time{f} = spike_time;
            
            %% decode movement-related variables (left vs. right, low vs. high)
            actual_rStart = func.alignGlmOutToTaskEvent(tV.evtRstart, time_bin, spike_bin_conv, 10, p.window + 4 * p.filter_sigma); % actual rate aligned to reachStart (to be decoded)
            % model left vs. right side
            trainLeRi = [[find(trainLe(pStartI)),zeros(sum(trainLe(pStartI)),1)]; ...
                        [find(trainRi(pStartI)), ones(sum(trainRi(pStartI)),1)]];
            trainLeRi_sort = sortrows(trainLeRi,1);
            model_rStart_LeRi = func.alignGlmOutToTaskEvent(tV.evtRstart(trainLeRi(:,1)), time_bin, expXprm1, 10, p.window + 4 * p.filter_sigma); % fitted rate aligned to reachStart
            model_leRi = func.group_stat2(model_rStart_LeRi, trainLeRi_sort(:,2)+1); % fitted mean left vs. mean right
            % get the posterior probability of being the left trial
            for t = 1:size(test_mv_pStartI(:,f),1)
                testDatC = cellfun(@(a) getTestDatCell(actual_rStart, a), test_mv_pStartI(t,f),'un', 0);
                decode.probLe{t,f,i_cell}   = cell2mat(cellfun(@(a) posteriorProb2(model_leRi(:,1), model_leRi(:,2), a), testDatC{:}, 'un', 0)); % type-by-fold-by-cell
            end
            clearvars t
            
            % model low vs. high torque
            trainLoHi = [[find(trainLo(pStartI)),zeros(sum(trainLo(pStartI)),1)]; ...
                        [find(trainHi(pStartI)), ones(sum(trainHi(pStartI)),1)]];
            trainLoHi_sort = sortrows(trainLoHi,1);
            model_rStart_LoHi = func.alignGlmOutToTaskEvent(tV.evtRstart(trainLoHi(:,1)), time_bin, expXprm1, 10, p.window + 4 * p.filter_sigma); % fitted rate aligned to reachStart
            model_loHi = func.group_stat2(model_rStart_LoHi, trainLoHi_sort(:,2)+1); % fitted mean left vs. mean right
            % get the posterior probability of being the low-torque trial
            for t = 1:size(test_mv_pStartI(:,f),1)
                testDatC = cellfun(@(a) getTestDatCell(actual_rStart, a), test_mv_pStartI(t,f),'un', 0);
                decode.probLo{t,f,i_cell}   = cell2mat(cellfun(@(a) posteriorProb2(model_loHi(:,1), model_loHi(:,2), a), testDatC{:}, 'un', 0)); % type-by-fold-by-cell
            end
            clearvars t
            
            %% decode rewarded or not
            train_Nr_R = sum([trainNoRwd trainRwd.*2],2);
            actual_Rwd = func.alignGlmOutToTaskEvent(tV.evtRwd, time_bin, spike_bin_conv, 10, p.window + 4 * p.filter_sigma);  % fitted rate aligned to trEnd+1000ms
            %actual_Rwd_cnt = func.alignGlmOutToTaskEvent(tV.evtRwd, time_bin, spike_bin_conv_cnt, 10, p.window + 4 * p.filter_sigma);
            % model no-reward vs. reward           
            model_Rwd_Nr_R = func.alignGlmOutToTaskEvent(tV.evtRwd(train_Nr_R>0), time_bin, expXprm1, 10, p.window + 4 * p.filter_sigma); % fitted rate aligned to reachStart
            model_Nr_R = func.group_stat2(model_Rwd_Nr_R, train_Nr_R(train_Nr_R>0)); % fitted mean left vs. mean right  
            
            % get the posterior probability of being the no-reward trial
            for t = 1:size(test_rwd(:,f),1)
                testDatC = cellfun(@(a) getTestDatCell(actual_Rwd, a), test_rwd(t,f),'un', 0);
                decode.probNoRwd{t,f,i_cell} = cell2mat(cellfun(@(a) posteriorProb2(model_Nr_R(:,1), model_Nr_R(:,2), a), testDatC{:}, 'un', 0)); % type-by-fold-by-cell
            end
            clearvars t
...
% save(fullfile('/Volumes/Beefcake/Junchol_Data/JS2p0/WR40_082019/Matfiles','glm_decoder_WR40_082019'),'rez','tV','p','decode')
