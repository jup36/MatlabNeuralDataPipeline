function hAx = rasterPlotCellSubplot(plotTitle, timeStampCells, subPlotSet, fullColorMap, alphaValue)
    ...
    trialLineLength = -0.2; % Adjust this value if needed to move the trial indicator lines further left
    ...
    for c = 1:numCellArrays
        ...
        % Draw the trial type indicator on the leftmost side
        line([trialLineLength, trialLineLength], [currentTrialOffset + 0.5, currentTrialOffset + numTrials + 0.5], 'Color', currentColor, 'LineWidth', 3);
    ...
    % assume each element is a cell of numbers (your original assumption)
    isEmpty = cellfun(@(x) isempty(cell2mat(x(:))), timeStampCells);
    vals = nan(size(timeStampCells));                  % prefill with NaN
    vals(~isEmpty) = cellfun(@(x) max(cell2mat(x(:))), timeStampCells(~isEmpty));

    xlim([trialLineLength, ceil(max(vals))]);
...
%     title(plotTitle)
%     set(gca, 'YDir', 'reverse', 'TickDir', 'out');
%     xlim([0, ceil(max(cell2mat(cellfun(@(x) max(cell2mat(x(:))), timeStampCells, 'UniformOutput', false))))]);
