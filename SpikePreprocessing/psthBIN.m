function [ binSpkOut, binSpk, currParams ] = psthBIN( fileInfo, regionInfo, neural, behav, baseEvt, binsize, psthWin, stableTrials, psthPlotFlag )
...
row = ceil(sqrt(size(neural,2)))-1; % # of rows
col = ceil(sqrt(size(neural,2)))+1; % # of columns
...
currParams.binEdges = linspace(-psthWin(1,1), psthWin(1,2), sum(psthWin)/binsize+1); % binEdges using the user-specified binsize
currParams.binEdges1ms = linspace(-psthWin(1,1), psthWin(1,2), sum(psthWin)+1);      % binEdges with 1ms binsize (to be used for detecting spike times)

% set a gaussian kernel to be convolved with the spike train (delta function)
currParams.smthParams.rise     = 1;   % gaussian kernel parameter
...
for u = 1:length(neural)   % # of units
    
    % get delta function for the whole spike train
    numAllSpks = length(neural{1,u});                       % # of spikes of the current unit
    delta = zeros(1,ceil(neural{1,u}(numAllSpks)));         % delta function of the whole spike train
    delta(1,ceil(neural{1,u})) = 1;                         % delta function of the whole spike train - put 1 at spike times
    tmpSmooth = conv(delta,currParams.filter.kernel,'same'); % convolution of the delta function with the Gaussian kernel
    
    tmpBinCountMat    = zeros(length(currEvt),length(currParams.binEdges)-1);     % temporary binned spike count matrix for the current unit
    tmpBinGCountMat   = zeros(length(currEvt),length(currParams.binEdges)-1);     % temporary binned spike count matrix (Gaussian convolved) for the current unit
    tmpBaseBinGCountMat = zeros(length(baseEvt),length(currParams.binEdges)-1);   % temporary binned spike count matrix (Gaussian convolved) for the baseline period
    tmpBinCountMat1ms = zeros(length(currEvt),length(currParams.binEdges1ms)-1);  
    ...
    for tr = 1:length(currEvt) % # of trial of the current event
        ...
        if currEvt(tr)+psthWin(1,2) < length(delta) % in case the current trial's psth doesn't go out of the end of the time series
            if currEvt(tr)-psthWin(1,1) > 0
                valEvtCnt = valEvtCnt + 1; % valid event count
                binSpk(u).SpkCountMat{valEvtCnt,1}  = sparse(histcounts(neural{1,u}, currEvt(tr) + currParams.binEdges));      % histogram bin counts; 
                binSpk(u).SpkGCountMat{valEvtCnt,1} = tmpSmooth(1,currEvt(tr)-psthWin(1,1)+1:currEvt(tr)+psthWin(1,2));       % event bin counts from the Gaussian convolved delta function;        % histogram bin counts; 
                binSpk(u).SpkTimes{valEvtCnt,1}     = find(histcounts(neural{1,u}, currEvt(tr) + currParams.binEdges1ms)==1);  % histogram bin counts with 1ms binsize
    ...
    binSpk(u).currParams = currParams;  % put currParams into the binSpk structure 
    ...
    binSpkOut(u).currParams = binSpk(u).currParams;
