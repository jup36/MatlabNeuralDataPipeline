function [ binSpkOut, binSpk, params ] = psthBINcellRmvArtifactNearZero( fileInfo, regionInfo, neural, behav, baseEvt, binsize, psthWin, stableTrials, psthPlotFlag )
...
for u = 1:size(neural,2)   % increment units
    ...
    currEvtRange = cell2mat(arrayfun(@(a) a-3:a+3, currEvt, 'un', 0)'); 
    delta(ismember(find(delta),currEvtRange))=0; 
    ...
    for tr = 1:length(currEvt) % # of trial of the current event
        ...
        if currEvt(tr)+psthWin(1,2) < length(delta) % in case the current trial's psth doesn't go out of the upper bound
            if currEvt(tr)-psthWin(1,1) > 0
                ...
                binSpk.SpkTimes{u,1}{tr,1}     = binSpk.SpkTimes{u,1}{tr,1}(abs((binSpk.SpkTimes{u,1}{tr,1}-params.psthWin(1)))>2); % remove artifacts near the event          
