...
gpfaResult = neuralTraj(gpfaRunNumber, gpfaDat, 'datFormat', 'spikes', 'method', 'gpfa', 'xDim', p.Results.gpfaDim, 'binWidth', p.Results.gpfaBinSize, 'kernSDList', p.Results.gpfaKernSD);
...
save(saveName, 'gpfaResult')

% Generate elementary plots for sanity check 
% plot3D(seqTrain, 'xorth', 'dimsToPlot', 1:3);
% plot(gpfaResult.currentParams.C(:,1))
% plot(seqTrain(1).xsm(3,:))
% plot(seqTrain(1).xorth(4,:))
% plotEachDimVsTime(seqTrain, 'xsm', gpfaResult.binWidth);
% plotEachDimVsTime(seqTrain, 'xorth', gpfaResult.binWidth);
