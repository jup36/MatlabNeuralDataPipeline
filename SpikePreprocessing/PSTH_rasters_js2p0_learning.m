function PSTH_rasters_js2p0_learning( filePath, fileInfo, probeDepth, varargin )
...
%p = parse_input_psth_js2p0(filePath, 'WR47_052421_learning', 3106, 'probeAngle', 10, 'numbSiteProbe',384, 'laserUsed', false) % when running line-by-line
...
elseif size(phyPath,1)>1
    im_I = cellfun(@(a) contains(a,p.Results.probeType(1:2), 'IgnoreCase', true), {phyPath(:).folder}); 
    prefix_I = cellfun(@(a) contains(a, '._', 'IgnoreCase', true),  {phyPath(:).name}); 
    phyPath = phyPath(im_I & ~prefix_I); 
end
