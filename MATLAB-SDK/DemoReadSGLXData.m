% =========================================================
% Simple program demonstrating how to read and manipulate
% SpikeGLX meta and binary data files using Bill utility
% functions and MATLAB structures.
%
...
% addpath('/Users/parkj/Desktop/SpikeGLX-master 2/MATLAB-SDK/')

function DemoReadSGLXData()

% Ask user for binary file
[binName,path] = uigetfile('*.bin', 'Select Binary File'); % bin file and path

% Parse the corresponding metafile
meta = ReadMeta(binName, path);     % get the meta data (structure)

% Get first one second of data
nSamp = SampRate(meta);                             % sampling rate
dataArray = ReadBin(0, nSamp, meta, binName, path); % read out the first 1-sec data from all saved channels 
...
function dataArray = ReadBin(samp0, nSamp, meta, binName, path)
    ...
    nFileSamp = str2double(meta.fileSizeBytes) / (2 * nChan); % this gives the total number of samples, which equals to time (s) x sampling rate
    samp0 = max(samp0, 0);
