function keyword_folder_paths = find_keyword_containing_folder(start_path, keyword)
    % Recursively search for all folders that contain the keyword in the specified start_path
    % Input:
    ...
    for i = 1:length(folder_info)
        if folder_info(i).isdir
            ...
            if contains(folder_info(i).name, keyword)
                ...
                keyword_folder_paths{end+1} = fullfile(start_path, folder_info(i).name);
            end
            
            % Recursively search this subdirectory
            subfolder_path = fullfile(start_path, folder_info(i).name);
            subfolder_keyword_paths = find_keyword_containing_folder(subfolder_path, keyword);
