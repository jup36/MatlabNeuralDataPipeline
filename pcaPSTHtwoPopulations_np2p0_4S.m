function [S,pc,sortedBinSpkCell] = pcaPSTHtwoPopulations_np2p0_4S( filePath, fileName, varName, varargin  )
...
% fileName = {'binSpkCountWR39_100819'}; 
% varName = 'pullStarts';

p = parse_input_psthPCA( filePath, fileName, varName, varargin );
%p = parse_input_psthPCA( filePath, fileName, varName, {'binSize',200,'stepSize',200,'binSizeZ',50,'useAllUnits',false,'PCsLogic',true,'PCs',5,'expVarLogic',false,'expVarCut',80,...
%'FRcut',2,'nanTrialCut',.1,'cmap','hot','cAxis',[-10 10], 'rmvNaNtrials', true , 'rmvNaNunits', false } );
...
for f = 1:length(fileName)
    ...
    figure; 
    imagescJP(file(f).S.binSpkZ(file(f).pc.pcUnitSort(1:end,1),:),cmap,p.Results.cAxis,'X',[Xs(1) Xs(end-1)],'cBar',true); 
