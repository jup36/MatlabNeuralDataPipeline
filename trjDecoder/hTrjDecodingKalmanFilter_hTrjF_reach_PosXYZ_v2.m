function hTrjDecodingKalmanFilter_hTrjF_reach_PosXYZ_v2(filePath, saveName, plotlogic)
...
for r = 1:size(s.dat.stateR,1)
    for c = 1:size(s.dat.stateR,2)
        if ~isempty(s.dat.stateR{r,c})
            s.dat.stateR{r,c} = s.dat.stateR{r,c}(1:3,:); % X, Y, Z position (row 1:3 for position, row 4:6 for velocity)
        end
    end
end
clearvars r c
...
%load(fullfile(filePath,strcat('rezKFdecodeHTrjCtxStrPos_reach_v2',saveName)),'s','corrRez','r2Rez') % last saved after training without stim trials 5/27 Wed 9pm
