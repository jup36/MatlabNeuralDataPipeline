function  [ctxRez, strRez, ctxstrRez] = leaveOneOutKFdecoder(stateD,ctxD,strD,laserI,resample)
%stateD = s.dat.stateR;
%ctxD = s.dat.spkCtxR; 
%strD = s.dat.spkStrR;
        ...
        for c = 1:size(ctxD,2) % column: position/torque pairs
            ...
            if valTrI(r,c)
                ...
                for cc = 1:size(valTrainI,2) % sample trials from each trial type
                    ...
                    trainTrs{1,cc} = tempValTr; %tempValTrRand(1:trainTrN); % take the set # of randomized trials from each trial type (column)
