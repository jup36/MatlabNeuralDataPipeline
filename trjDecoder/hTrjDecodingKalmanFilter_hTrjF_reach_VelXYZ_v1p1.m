function hTrjDecodingKalmanFilter_hTrjF_reach_VelXYZ_v1p1(filePath, saveName, plotlogic)
    ...
    for r = 1:size(s.dat.spkCtxR,1) % row: trials
        for c = 1:size(s.dat.spkCtxR,2) % column: position/torque pairs
            ...
            if valTrI(r,c)
                ...
                currTrainTrs = cell(1,size(trainI,2));
                ...
                for cc = 1:size(valTrainI,2) % sample trials from each trial type
                    ...
                    currTrainTrs{1,cc} = tempValTr; %tempValTrRand(1:trainTrN); % take the set # of randomized trials from each trial type (column)
                    currTrainState = [currTrainState; s.dat.stateR(currTrainTrs{1,cc},cc)]; % concatanate randomly selected trials from each type to construct the train data matrix
                    currTrainCtx = [currTrainCtx; cellfun(@(a) a(ctxI,:), s.dat.spkCtxR(currTrainTrs{1,cc},cc), 'un', 0)]; % ctx spike mat with cells resampled to match # of cells
                    currTrainStr = [currTrainStr; cellfun(@(a) a(strI,:), s.dat.spkStrR(currTrainTrs{1,cc},cc), 'un', 0)]; % str spike mat with cells resampled to match # of cells
                    currTrainCtxStr = [currTrainCtxStr; cellfun(@(a,b) [a(ctxI,:); b(strI,:)], s.dat.spkCtxR(currTrainTrs{1,cc},cc), s.dat.spkStrR(currTrainTrs{1,cc},cc), 'un', 0)]; 
                end
                clearvars cc
                ...
                for b = 1:curTrLength % # of timebins
                    ...
                    curObsX_ctxstr = [s.dat.spkCtxR{testI}(ctxI,b);s.dat.spkStrR{testI}(strI,b)]; % take the current ctx spikes bin-by-bin
...
for rr = 1:size(s.dat.estStateCtxMean,1) % trials (row)
    for cc = 1:size(s.dat.estStateCtxMean,2) % trial-types (column)
        ...
        s.dat.stateRCtxStr{rr,cc} = nanmedian(cell2mat(s.dat.estStateCtxStrMean(rr,cc,:)),3); % average across ctxstr resampled trials
        tmpCtxStr = s.dat.stateRCtxStr{rr,cc};
...
[corrRez.ctx,r2Rez.ctx] =  trjDecodeEvalByTrialType(s.dat.stateR, s.dat.stateRCtx); 
[corrRez.str,r2Rez.str] = trjDecodeEvalByTrialType(s.dat.stateR, s.dat.stateRStr); 
[corrRez.ctxstr,r2Rez.ctxstr] = trjDecodeEvalByTrialType(s.dat.stateR, s.dat.stateRCtxStr); 

save(fullfile(filePath,strcat('rezKFdecodeHTrjCtxStrVel_reach_v2',saveName)),'s','corrRez','r2Rez') % last saved after training without stim trials 5/27 Wed 9pm
