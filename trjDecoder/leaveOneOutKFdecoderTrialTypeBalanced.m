...
estStateNanCtx=sum(cell2mat(cellfun(@(a) sum(sum(isnan(a))),estStateCtxMean,'un',0)),3)==0;
[ctxRez.est,ctxRez.estCut,ctxRez.estInt] = getRepresentativeStateEst(stateD,estStateCtxMean,valTrI&estStateNanCtx);
estStateNanStr=sum(cell2mat(cellfun(@(a) sum(sum(isnan(a))),estStateStrMean,'un',0)),3)==0;
[strRez.est,strRez.estCut,strRez.estInt] = getRepresentativeStateEst(stateD,estStateStrMean,valTrI&estStateNanStr);
estStateNanCtxStr=sum(cell2mat(cellfun(@(a) sum(sum(isnan(a))),estStateCtxStrMean,'un',0)),3)==0;
[ctxstrRez.est,ctxstrRez.estCut,ctxstrRez.estInt] = getRepresentativeStateEst(stateD,estStateCtxStrMean,valTrI&estStateNanCtxStr);
...
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
    function [representState,representStateCut,representIntState] = getRepresentativeStateEst(refState,estState,valTrId)
        ...
        %valTrId = valTrI;
        estStateNan=sum(cell2mat(cellfun(@(a) sum(sum(isnan(a))),estState,'un',0)),3)==0;
