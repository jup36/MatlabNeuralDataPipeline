function hTrjDecodingKalmanFilter_hTrjF_reach_VelXYZ_v2(filePath, saveName, plotlogic)
...
%%%%%%%%%%%%%%% NOTE THAT s.dat.stateR contains hand trajectories already
        ...
        for c = 1:size(s.dat.spkCtxR,2) % column: position/torque pairs
            ...
            if valTrI(r,c)
                ...
                for cc = 1:size(valTrainI,2) % sample trials from each trial type
                    ...
                    currTrainTrs{1,cc} = tempValTrRand(1:trainTrN); % take the set # of randomized trials from each trial type (column)
...
for rr = 1:size(s.dat.estStateCtxMean,1) % trials (row)
    for cc = 1:size(s.dat.estStateCtxMean,2) % trial-types (column)
        ...
        if ~isempty(s.dat.stateR{rr,cc})
            s.dat.dTrjCtx{rr,cc} = sqrt((s.dat.stateR{rr,cc}-s.dat.stateRCtx{rr,cc}).^2);
            s.dat.dTrjStr{rr,cc} = sqrt((s.dat.stateR{rr,cc}-s.dat.stateRStr{rr,cc}).^2);
            s.dat.dTrjStrStr{rr,cc} = sqrt((s.dat.stateR{rr,cc}-s.dat.stateRCtxStr{rr,cc}).^2);
        else
            s.dat.dTrjCtx{rr,cc} = [];
            s.dat.dTrjStr{rr,cc} = [];
...
%load(fullfile(filePath,strcat('rezKFdecodeHTrjCtxStrVel_reach_v2',saveName)),'s','corrRez','r2Rez') % last saved after training without stim trials 5/27 Wed 9pm
