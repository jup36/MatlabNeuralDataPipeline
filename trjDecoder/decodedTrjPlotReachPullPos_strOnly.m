function decodedTrjPlotReachPullPos_strOnly(filePath)
...
preDir = dir('preprocessKFdecodeHTrjStr_reachPull_hTrjF_*'); 
preS = load(fullfile(preDir.folder,preDir.name),'s'); 
preS = preS.('s'); 

kfDirR = dir('rezKFdecodeHTrjStrPosVel_reach_*'); 
sR = load(fullfile(kfDirR.folder,kfDirR.name),'s');
sR = sR.('s'); 

kfDirP = dir('rezKFdecodeHTrjStrPosVel_pull_*'); 
sP = load(fullfile(kfDirP.folder,kfDirP.name),'s');
sP = sP.('s'); 

%valTrI = cell2mat(cellfun(@(a) ~isempty(a), sR.dat.spkStrR, 'un', 0)); % valid trials
