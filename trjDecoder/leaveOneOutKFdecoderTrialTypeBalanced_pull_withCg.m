function  rez = leaveOneOutKFdecoderTrialTypeBalanced_pull_withCg(stateD, s, resample)
%stateD = tmpState;

nd = {};  % container cell for all neural data
counter = 0;
if isfield(s.dat, 'spkCtxP')
    counter = counter + 1;
    nd{1, counter} = 'ctx';
    nd{2, counter} = s.dat.spkCtxP;
...
if isfield(s.dat, 'spkStrP')
    counter = counter + 1;
    nd{1, counter} = 'str';
    nd{2, counter} = s.dat.spkStrP;
    nd{3, counter} = find(s.strI);
    nd{4, counter} = mode(cellfun(@(a) size(a,1), nd{2, counter}(valTrI)));
end

if isfield(s.dat, 'spkCgP')
    counter = counter + 1;
    nd{1, counter} = 'cg';
    nd{2, counter} = s.dat.spkCgP;
    if ~exist('valTrI', 'var')
        valTrI = cell2mat(cellfun(@(a) ~isempty(a), nd{2, counter}, 'un', 0)); % valid trials
    end
    nd{4, counter} = mode(cellfun(@(a) size(a,1), nd{2, counter}(valTrI)));
        ...
        for c = 1:size(valTrI,2) % column: position/torque pairs
            ...
            if valTrI(r,c)
                ...
                for cc = 1:size(valTrainI,2)  % sample trials from each trial type
                    ...
                    elseif ~ctx_record && ~str_record && cg_record
                        train_cg = [train_cg; train_obs_tt{1}];  % cg
                    end
                end
                clearvars cc
                ...
                if ctx_record && str_record && cg_record
                    ...
                    % CTX-STR decoder
                    [rez.estStateCtxStrM{r,c,i}, rez.estStateCtxStrCov{r,c,i}, rez.coeff_CtxStr{r,c,i}] = train_test_KF(trainState, train_ctx_str, nd_ctx_str, stateD{testI}, testI); % Ctx & Str
                ...
                elseif ~ctx_record && ~str_record && cg_record
                    % Cg decoder
                    [rez.estStateCgM{r,c,i}, rez.estStateCgCov{r,c,i}, ~] = train_test_KF(trainState, train_cg, nd{2,1}, stateD{testI}, testI, ...
                        nd{3,1}, nd{3,1}, length(nd{3,1})); % Cg
                end
            end
        end
    end
...
if ctx_record && str_record && cg_record  % Ctx, Str, Cg
    ...
    [rez.rpr_est_ctx1, rez.rpr_est_cut_ctx1, rez.rpr_est_intp_ctx1] = getRepresentativeStateEst(stateD, rez.estStateCtxM_cg, valTrI);  % neuron # matched with Cg    
    
    [rez.rpr_est_str, rez.rpr_est_cut_str, rez.rpr_est_intp_str] = getRepresentativeStateEst(stateD, rez.estStateStrM, valTrI);
    [rez.rpr_est_str1, rez.rpr_est_cut_str1, rez.rpr_est_intp_str1] = getRepresentativeStateEst(stateD, rez.estStateStrM_cg, valTrI);  % neuron # matched with Cg    
