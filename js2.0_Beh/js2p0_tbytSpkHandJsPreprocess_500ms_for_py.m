function js2p0_tbytSpkHandJsPreprocess_500ms_for_py(filePath)
...
for t = 1:size(jkvt,2)
    ...
    if strcmpi(jkvt(t).trialType, 'sp') % a successful pull trial
        if ~isempty(jkvt(t).pullStarts) && ~isempty(jkvt(t).pullStops) % ~isempty(jkvt(t).rStartToPull) % if there's rStartToPull
            ss(t).timeAlign = jkvt(t).pullStarts; % jkvt(t).rStartToPull; % align to reach start to pull
            ss(t).evtAlign  = 'pStart'; % 'rStart';
    ...
    ss(t).spkTimeBins = ss(t).timeAlign + (spkBin(1):stepSize:spkBin(end-1));
    spkTime1msBins = ss(t).timeAlign + (spkBin(1):spkBin(end-1));
    
    if exist('spkTimesCellCTX')==1
       ss(t).unitTimeBCtx = psthBINcellPerTrial_step( spkTimesCellCTX, ss(t).timeAlign, binSize, stepSize, [abs(spkBin(1)) abs(spkBin(end))]); % binned spikeCounts aligned to this trial
    end
    if exist('spkTimesCellSTR')==1
       ss(t).unitTimeBStr = psthBINcellPerTrial_step( spkTimesCellSTR, ss(t).timeAlign, binSize, stepSize, [abs(spkBin(1)) abs(spkBin(end))]); % binned spikeCounts aligned to this trial
    end
    if exist('spkTimesCellCg')==1
       ss(t).unitTimeBCg = psthBINcellPerTrial_step( spkTimesCellCg, ss(t).timeAlign, binSize, stepSize, [abs(spkBin(1)) abs(spkBin(end))]); % binned spikeCounts aligned to this trial
    end
    ...
    if ~isempty(hTrj{t}) && ~isempty(ss(t).timeAlign) % if hTrj available
        ...
        ss(t).hInitPos = nanmedian(ss(t).hTrjB(:,1:max(1,floor(abs(spkBin(1))/binSize/2))),2); % initial hand position
...
save(fullfile(filePath,strcat('js2p0_tbytSpkHandJsTrjBin_500ms_',saveName)),'ss','jkvt','trI','spkTimesCell*','depth*')
