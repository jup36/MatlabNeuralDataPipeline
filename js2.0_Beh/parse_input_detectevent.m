  function p = parse_input_detectevent(filePath, timeseries, sampFreq, detectTimeout, saveName, vargs)
        ...
        default_detectLater = 0;        % detect events later than a certain timepoint to prevent detection of premature events
        default_detectEarlier = 0; % detect events earlier than a certain timepoint to preclude late events
        ...
        default_freqForConvolution = [1000, 5000]; % frequency to be used for convolution and detection (these are the low frequency components of the two tone)
