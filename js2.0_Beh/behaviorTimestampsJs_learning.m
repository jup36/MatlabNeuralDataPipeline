...
if ~isempty(dir(fullfile(p.Results.filePath,'gainCorrectRawTraces.mat'))) && p.Results.reReadBin==false % if the gainCorrectRawTraces.mat file already exists in the filePath
    load(fullfile(p.Results.filePath,'gainCorrectRawTraces.mat'), 'lick', 'trStart', 'reward', 'trEnd', 'camTrig', 'encodeA', 'encodeB', 'laser', 'plaser', 'synch' ) % if there are gaincorrectedrawtraces already saved, just load them
else
    ...
    syncCh = channels{1}+p.Results.syncCh;
    ...
    synch      = zeros(1,floor(totalTimeSecs*25000)); 
    ...
    for k = 0:totalTimeSecs-1 % read second-by-second incrementally to avoid a memory issue
        ...
        synch(1,k*25000+1:(k+1)*25000) = tempSync; 
        fprintf('processed %d\n', k+1)
    end
    clearvars k
    ...
    if strcmp(meta.typeThis, 'imec') % in case recording via imec
        ...
        synch = GainCorrectIM(synch, 1, meta);
        ...
        synch = GainCorrectNI(synch, 1, meta);
    end
    clearvars temp*
    save('gainCorrectRawTraces', 'trStart', 'camTrig', 'reward', 'trEnd', 'encodeA', 'encodeB', 'lick', 'laser', 'plaser', 'synch')
end
...
if ~isempty(dir(fullfile(p.Results.filePath,'evtIndices.mat'))) && p.Results.reReadBin==false % if the gainCorrectRawTraces.mat file already exists in the filePath
    ...
    % detect licks
    % preprocess the lick signal 
    lick1k = decimate(lick,nSamp/1000); 
    lick1kDiff = diff([lick1k(1) lick1k]); 
    lickPad = nanmedian(lick1kDiff); 
    lickArtiIdx = detecteventbythreshold(lick1kDiff, 1000, 20, 'stdFactor',3, 'plotRez',false, 'chunkPulses', false); % lick, detect licks after downsampling (there seems to be some denoising effect with decimation
    lickArtifacts = arrayfun(@(a) max(0,a-20):min(length(lick1k),a+20), lickArtiIdx,'un', 0);
    for cc = 1:length(lickArtifacts)
        lick1kDiff(1,lickArtifacts{cc})= repmat(lickPad,1,length(lickArtifacts{cc})); 
        formatSpec = 'processing %d segments out of %d total\n';
        fprintf(formatSpec, cc, length(lickArtifacts)); 
    end
    lickIdx = detecteventbythreshold( lick1kDiff, 1000, 20, 'stdFactor',3, 'plotRez',false, 'chunkPulses', false); % lick, detect licks after downsampling (there seems to be some denoising effect with decimation, see /Volumes/RAID2/parkj/NeuralData/js2.0/WR25/101718/lickSignalExample_1kHzVS25kHz.fig as an example)
    ...
    % detect camTriggers
    [camTrigRiseIdx, camTrigFallIdx, camPulseTrainIdx] = detecteventbythreshold(camTrig, 25000, 1.5, 'stdFactor', 1, 'plotRez',false, 'chunkPulses', true, 'chunkInterval', 2000, 'correctLongPulse', true); % camera trigger
...
for t = 1:length(trStartIdx) % increment trials
    if ~isempty(find(trEndIdx>trStartIdx(t),1)) % if there's a trEnd
        ...
        elseif trStartIdx(t)-2*nSamp+trBaseLatestStillTime <= jsTime25k(t).trStart+(nSamp/10/2) % if there's a joystick still point within 50 ms from the trStart signal
            ...
            if sum(jsTime25k(t).pull_torque==p.Results.meanMass(1,:))==1
                tempMass = p.Results.meanMass(2,jsTime25k(t).pull_torque==p.Results.meanMass(1,:));
            elseif sum(jsTime25k(t).pull_torque==p.Results.meanMass(1,:))==0
                tempMass = nanmean([max(p.Results.meanMass(2,jsTime25k(t).pull_torque>p.Results.meanMass(1,:))),min(p.Results.meanMass(2,jsTime25k(t).pull_torque<p.Results.meanMass(1,:)))]);
            end
