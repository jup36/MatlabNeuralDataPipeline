function jsKinematicsAnalysisIndividualSessions(filePath)
...
%% sort pull torques and reach positions and set colors for each pair
...
for t = 1:length(S)
    ...
    if actIdx(t)
        tempColor = rchP1TrqC(S(t).pull_torque*S(t).reachP1==rchP1TrqPairs,:); 
        %tempColor = pullTqsCmap(S(t).pull_torque == pullTqs,:);
        if S(t).rewarded
            plot((S(t).trJsReady+S(t).rt)/1000, S(t).rt,'o','MarkerFaceColor',tempColor,'MarkerEdgeColor','none','MarkerSize',10);
            rtCollect = [rtCollect; S(t).trJsReady+S(t).rt, S(t).rt];
        else % not rewarded
            plot((S(t).trJsReady+S(t).rt)/1000, S(t).rt,'o','MarkerEdgeColor','none','MarkerSize',10);
            rtCollect = [rtCollect; S(t).trJsReady+S(t).rt, S(t).rt];
        end
    end
end
...
for tB = 1:4 %unique([S(:).tBound])
    tempTr = find([S(:).tBound]==tB,1); 
    tempColor = rchP1TrqC(S(tempTr).pull_torque*S(tempTr).reachP1==rchP1TrqPairs,:); 
    ...
    bRt = bar(nanmean(rtCell{tB,1})/1000,nanmean(rtCell{tB,2}),'barWidth',30,'EdgeColor','none');
...
for t = 1:length(S)
    if strcmp(S(t).trialType,'sp')
        ...
        plot((S(t).trJsReady+S(t).rt)/1000, S(t).rd,'o','MarkerFaceColor',tempColor,'MarkerEdgeColor','none','MarkerSize',10);
    end
end
clearvars t
...
for tB = unique([S(:).tBound])
    ...
    bRd = bar(nanmean(rdCell{tB,1})/1000,nanmean(rdCell{tB,2}),'barWidth',30,'EdgeColor','none');
...
for t = 1:length(S)
    ...
    if spIdx(t)&&actIdx(t)
        ...
        plot((S(t).trJsReady+S(t).rt)/1000, -S(t).maxForcePull,'o','MarkerFaceColor',tempColor,'MarkerEdgeColor','none','MarkerSize',10);
    elseif toIdx(t)&&actIdx(t)
        ...
        mfCollect = [mfCollect; t, S(t).trJsReady+S(t).rt, S(t).maxForcePull];
        plot((S(t).trJsReady+S(t).rt)/1000, -S(t).maxForcePull,'o','MarkerEdgeColor','none','MarkerSize',10);
...
for tB = unique([S(:).tBound])
    ...
    bMf = bar(nanmean(maCell{tB,1})/1000,nanmean(maCell{tB,2}),'barWidth',30,'EdgeColor','none');
...
%         tempX1 = S(reachP1shiftPts(s)).trStart;
%         tempX2 = max(S(reachP1shiftPts(s+1)-1).trEnd, S(reachP1shiftPts(s+1)-1).rewardT);
%         fill([tempX1 tempX2 tempX2 tempX1]./1000, [-10 -10 10000 10000], tempColor, 'EdgeColor','none');
%     elseif s==length(reachP1shiftPts)
%         tempColor = reachPsCmap(reachP1s==S(reachP1shiftPts(s)).reachP1,:);
%         tempX1 = S(reachP1shiftPts(s)).trStart;
%         tempX2 = max(S(length(S)).trEnd, S(length(S)).rewardT);
...
for t = 1:length(S)
    ...
    if spIdx(t)&&actIdx(t)
        %tempColor = pullTqsCmap(S(t).pull_torque == pullTqs,:);
        S(t).maxAccelPull = S(t).movKins.maxForce/S(t).movKins.mass.*100;
        maCollect = [maCollect; t, S(t).trJsReady+S(t).rt, S(t).maxAccelPull];
        plot((S(t).trJsReady+S(t).rt)/1000, -S(t).maxAccelPull,'o','MarkerFaceColor',tempColor,'MarkerEdgeColor','none','MarkerSize',10);
    elseif toIdx(t)&&actIdx(t)
        %tempColor = pullTqsCmap(S(t).pull_torque == pullTqs,:);
        if ~isempty(find(S(t).movKins.periodicAbsVelSum(1:1000)>50,1,'last'))
            absltStillPt = find(S(t).movKins.periodicAbsVelSum(1:1000)>50,1,'last')+50;
        ...
        S(t).maxAccelPull = nanmin(S(t).movKins.forceMN(1,absltStillPt:end))/S(t).movKins.mass.*100;
        maCollect = [maCollect; t, S(t).trJsReady+S(t).rt, S(t).maxAccelPull];
        plot((S(t).trJsReady+S(t).rt)/1000, -S(t).maxAccelPull,'o','MarkerEdgeColor','none','MarkerSize',10);
        
    elseif pmppIdx(t)&&actIdx(t)
        %tempColor = pullTqsCmap(S(t).pull_torque == pullTqs,:);
        S(t).maxAccelPull = S(t).movKins.pull.maxForce/S(t).movKins.mass.*100;
        maCollect = [maCollect; t, S(t).trJsReady+S(t).rt, S(t).maxAccelPull];
        plot((S(t).trJsReady+S(t).rt)/1000, -S(t).maxAccelPull,'o','MarkerEdgeColor','none','MarkerSize',10);
    end
end
...
clearvars tB
hold off;
ylabel('max Accel (cm/s^2)')
xlabel('Time(s)')
axis tight
xlim([0 850])
ylim([0 2500])%-min(maCollect(:,3))+5])
...
%         tempX1 = S(reachP1shiftPts(s)).trStart;
%         tempX2 = max(S(reachP1shiftPts(s+1)-1).trEnd, S(reachP1shiftPts(s+1)-1).rewardT);
%         fill([tempX1 tempX2 tempX2 tempX1]./1000, [-10 -10 100 100], tempColor, 'EdgeColor','none');
%     elseif s==length(reachP1shiftPts)
%         tempColor = reachPsCmap(reachP1s==S(reachP1shiftPts(s)).reachP1,:);
%         tempX1 = S(reachP1shiftPts(s)).trStart;
%         tempX2 = max(S(length(S)).trEnd, S(length(S)).rewardT);
...
for t = 1:length(S)
    ...
    if spIdx(t)&&actIdx(t) % successful pull trials
        %tempColor = pullTqsCmap(S(t).pull_torque == pullTqs,:);
        S(t).maxVelPull = S(t).movKins.pullMaxVel./10;
        plot((S(t).trJsReady+S(t).rt)/1000, -S(t).maxVelPull,'o','MarkerFaceColor',tempColor,'MarkerEdgeColor','none','MarkerSize',10);
    elseif toIdx(t)&&actIdx(t) % timeout trials
        ...
        S(t).maxVelPull = nanmin(S(t).movKins.smJsVel(1,absltStillPt:end)./10);
        plot((S(t).trJsReady+S(t).rt)/1000, -S(t).maxVelPull,'o','MarkerEdgeColor','none','MarkerSize',10);
    elseif pmppIdx(t)&&actIdx(t) % premature pull and push
        %tempColor = pullTqsCmap(S(t).pull_torque == pullTqs,:);
        S(t).maxVelPull = S(t).movKins.pull.maxVel./10;
        plot((S(t).trJsReady+S(t).rt)/1000, -S(t).maxVelPull,'o','MarkerEdgeColor','none','MarkerSize',10);
    end
...
for tB = unique([S(:).tBound])
    ...
    bMf = bar(nanmean(mvCell{tB,1})/1000,nanmean(mvCell{tB,2}),'barWidth',30,'EdgeColor','none');
    set(bMf,'FaceColor',tempColor)
