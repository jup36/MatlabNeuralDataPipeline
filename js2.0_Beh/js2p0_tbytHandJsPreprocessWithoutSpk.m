function js2p0_tbytHandJsPreprocessWithoutSpk(filePath,saveName)
...
% Then for each trial, hand/joystick trajectories (from DLC) and joystick kinetics 
% (force applied from joystick encoder) are acquired, if available, aligned to an event 
% ('reachStart' or 'pullStart' or 'trJoystickReady') depending on the trial type. 
% Additionally, some useful trial-by-trial behavioral variables are computed for future decoding
...
%filePath = '/Volumes/Beefcake/Junchol_Data/JS2p0/WR37_022219/Matfiles';
cd(filePath)
...
% get some timestamps to align trajectories to (also included in 'PSTH_rasters_js2p0.m')
for t = 1:length(jkvt)
...
% extrapolation of hand trajectories, if needed
