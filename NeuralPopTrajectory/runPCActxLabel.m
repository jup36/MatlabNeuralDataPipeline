function [ pcaResult, pcaDat ] = runPCActxLabel( filePath, fileName, saveNameTag, varargin )
...
% p = parse_input_runPCA( filePath, fileName, saveNameTag, { 'frHighPass', 1, 'frLowPass', 50, 'timeRange', [-3e3 2e3], 'pcaBinSize', 50, 'pcaDim', 5, 'pcaKernSD', 100, 'fullCrossVal', false, 'xcorThresholdPer', 0.2, 'crossValFolds', 0,  'baseSubtrt', true, 'sqrtSC', false } ); % use this when running line-by-line
...
if ~isempty(p.Results.unitIdxFile) && exist(fullfile(p.Results.filePath,p.Results.unitIdxFile),'file')==2 % if there's a unitIdxFile specifically input, and the file exists
    ...
    unitIdxPCA = unitMeanFR > p.Results.frHighPass & unitMeanFR < p.Results.frLowPass & xcorUnitIdx & tempMCtxIall'; % units to be used for pca
end
