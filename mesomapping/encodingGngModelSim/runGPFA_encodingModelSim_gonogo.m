...
%     trialgroup_spec_C{1}{end+1}= bin1msSpkCountMat(max(trialgroup.spec(i).psth(:, 1:1700, 1), 0), 50, 50); % 50-ms bin
%     trialgroup_spec_C{2}{end+1}= bin1msSpkCountMat(max(trialgroup.spec(i).psth(:, 1:1700, 2), 0), 50, 50);
%     trialgroup_spec_C{3}{end+1}= bin1msSpkCountMat(max(trialgroup.spec(i).psth(:, 1:1700, 3), 0), 50, 50);
%     trialgroup_spec_C{4}{end+1}= bin1msSpkCountMat(max(trialgroup.spec(i).psth(:, 1:1700, 4), 0), 50, 50);
% end
% 
% trialgroup_selt_C = cell(4, 1);
% for i = 1:length(trialgroup.selt)
%     trialgroup_selt_C{1}{end+1}= bin1msSpkCountMat(max(trialgroup.selt(i).psth(:, 1:1700, 1), 0), 50, 50); % 50-ms bin
%     trialgroup_selt_C{2}{end+1}= bin1msSpkCountMat(max(trialgroup.selt(i).psth(:, 1:1700, 2), 0), 50, 50);
%     trialgroup_selt_C{3}{end+1}= bin1msSpkCountMat(max(trialgroup.selt(i).psth(:, 1:1700, 3), 0), 50, 50);
%     trialgroup_selt_C{4}{end+1}= bin1msSpkCountMat(max(trialgroup.selt(i).psth(:, 1:1700, 4), 0), 50, 50);
% end
...
% specification model
saveNameSpec = fullfile(filePath, 'gpfa_spec_short');
...
datSpec_tt1_mean = mean(cell2mat(reshape({datSpec(1:10).spikes}, [1, 1, 10])), 3);
datSpec_tt1_meanS.trialId = 1:10; 
datSpec_tt1_meanS.spikes = datSpec_tt1_mean ; 
gpfaRezSpec_tt1_mean = projYtoGetXsm(fullfile(saveNameSpec, strcat(sprintf('%s_xDim%02d', 'gpfa', 5))), datSpec_tt1_meanS);  % trial type 1

gpfaRezSpec_tt2 = projYtoGetXsm(fullfile(saveNameSpec, strcat(sprintf('%s_xDim%02d', 'gpfa', 5))), datSpec(11:20)); % trial type 2
datSpec_tt2_mean = mean(cell2mat(reshape({datSpec(11:20).spikes}, [1, 1, 10])), 3);
datSpec_tt2_meanS.trialId = 11:20; 
datSpec_tt2_meanS.spikes = datSpec_tt2_mean ; 
gpfaRezSpec_tt2_mean = projYtoGetXsm(fullfile(saveNameSpec, strcat(sprintf('%s_xDim%02d', 'gpfa', 5))), datSpec_tt2_meanS);  % trial type 2

gpfaRezSpec_tt3 = projYtoGetXsm(fullfile(saveNameSpec, strcat(sprintf('%s_xDim%02d', 'gpfa', 5))), datSpec(21:30)); % trial type 3
datSpec_tt3_mean = mean(cell2mat(reshape({datSpec(21:30).spikes}, [1, 1, 10])), 3);
datSpec_tt3_meanS.trialId = 21:30; 
datSpec_tt3_meanS.spikes = datSpec_tt3_mean ; 
gpfaRezSpec_tt3_mean = projYtoGetXsm(fullfile(saveNameSpec, strcat(sprintf('%s_xDim%02d', 'gpfa', 5))), datSpec_tt3_meanS);  % trial type 3

gpfaRezSpec_tt4 = projYtoGetXsm(fullfile(saveNameSpec, strcat(sprintf('%s_xDim%02d', 'gpfa', 5))), datSpec(31:40)); % trial type 4
datSpec_tt4_mean = mean(cell2mat(reshape({datSpec(31:40).spikes}, [1, 1, 10])), 3);
datSpec_tt4_meanS.trialId = 31:40; 
datSpec_tt4_meanS.spikes = datSpec_tt4_mean ; 
gpfaRezSpec_tt4_mean = projYtoGetXsm(fullfile(saveNameSpec, strcat(sprintf('%s_xDim%02d', 'gpfa', 5))), datSpec_tt4_meanS);  % trial type 4

colorC = {[60, 85, 162]./255, [93, 117, 179]./255, [229, 136, 141]./255, [233, 44, 44]./255}; 
plot3D_seqCells_withMean_color({gpfaRezSpec_tt1, gpfaRezSpec_tt2, gpfaRezSpec_tt3, gpfaRezSpec_tt4}, ...
    {gpfaRezSpec_tt1_mean, gpfaRezSpec_tt2_mean, gpfaRezSpec_tt3_mean, gpfaRezSpec_tt4_mean}, colorC, 10);
view(az, el);
print(fullfile(saveNameSpec, "gpfa_xDim05_specification_model"), '-dpdf', '-bestfit', '-vector')


%% GPFA selection model
saveNameSelt = fullfile(filePath, 'gpfa_selt_short');
...
datSelt_tt1_mean = mean(cell2mat(reshape({datSelt(1:10).spikes}, [1, 1, 10])), 3);
datSelt_tt1_meanS.trialId = 1:10; 
datSelt_tt1_meanS.spikes = datSelt_tt1_mean ; 
gpfaRezSelt_tt1_mean = projYtoGetXsm(fullfile(saveNameSelt, strcat(sprintf('%s_xDim%02d', 'gpfa', 5))), datSelt_tt1_meanS);  % trial type 1

gpfaRezSelt_tt2 = projYtoGetXsm(fullfile(saveNameSelt, strcat(sprintf('%s_xDim%02d', 'gpfa', 5))), datSelt(11:20)); % trial type 2
datSelt_tt2_mean = mean(cell2mat(reshape({datSelt(11:20).spikes}, [1, 1, 10])), 3);
datSelt_tt2_meanS.trialId = 11:20; 
datSelt_tt2_meanS.spikes = datSelt_tt2_mean ; 
gpfaRezSelt_tt2_mean = projYtoGetXsm(fullfile(saveNameSelt, strcat(sprintf('%s_xDim%02d', 'gpfa', 5))), datSelt_tt2_meanS);  % trial type 2

gpfaRezSelt_tt3 = projYtoGetXsm(fullfile(saveNameSelt, strcat(sprintf('%s_xDim%02d', 'gpfa', 5))), datSelt(21:30)); % trial type 3
datSelt_tt3_mean = mean(cell2mat(reshape({datSelt(21:30).spikes}, [1, 1, 10])), 3);
datSelt_tt3_meanS.trialId = 21:30; 
datSelt_tt3_meanS.spikes = datSelt_tt3_mean ; 
gpfaRezSelt_tt3_mean = projYtoGetXsm(fullfile(saveNameSelt, strcat(sprintf('%s_xDim%02d', 'gpfa', 5))), datSelt_tt3_meanS);  % trial type 3

gpfaRezSelt_tt4 = projYtoGetXsm(fullfile(saveNameSelt, strcat(sprintf('%s_xDim%02d', 'gpfa', 5))), datSelt(31:40)); % trial type 4
datSelt_tt4_mean = mean(cell2mat(reshape({datSelt(31:40).spikes}, [1, 1, 10])), 3);
datSelt_tt4_meanS.trialId = 31:40; 
datSelt_tt4_meanS.spikes = datSelt_tt4_mean ; 
gpfaRezSelt_tt4_mean = projYtoGetXsm(fullfile(saveNameSelt, strcat(sprintf('%s_xDim%02d', 'gpfa', 5))), datSelt_tt4_meanS);  % trial type 4

plot3D_seqCells_withMean_color({gpfaRezSelt_tt1, gpfaRezSelt_tt2, gpfaRezSelt_tt3, gpfaRezSelt_tt4}, ...
    {gpfaRezSelt_tt1_mean, gpfaRezSelt_tt2_mean, gpfaRezSelt_tt3_mean, gpfaRezSelt_tt4_mean}, colorC, 10);
[az, el] = view;
print(fullfile(saveNameSelt, "gpfa_xDim05_selection_model"), '-dpdf', '-bestfit', '-vector')
...
% 
% plot3D_pcaSeqCells_color(pcaXpostC, {[0 0 1], [0 1 1], [1 0 1], [1 0 0]}, 10);
% viewAngle = view;  % Get the current view angle of the first plot
