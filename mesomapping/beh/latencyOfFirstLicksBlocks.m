function [lat] = latencyOfFirstLicksBlocks(tbytDat, trRwdI, trPnsI, blocks, cueWin)

    % First lick latency across blocks
    lickTimeC = cellfun(@(a, b) a-b, {tbytDat.Lick}, {tbytDat.evtOn}, 'un', 0);

    lickTimeAfterCueC = cellfun(@(a) a(a>=cueWin(2)), lickTimeC, 'UniformOutput', false);

    lickCounts = cell2mat(cellfun(@length, lickTimeAfterCueC, 'UniformOutput', false));
    valTrI = lickCounts > 0;
    ...
    lat.allFstLat(valTrI) = cellfun(@(a) a(1), lickTimeAfterCueC(valTrI), 'UniformOutput', false);
    lat.rwdFstLat(valRwdTrI) = cellfun(@(a) a(1), lickTimeAfterCueC(valRwdTrI), 'UniformOutput', false);
...
%     lat.rwdFstLatMiss(valRwdTrI) = cellfun(@(a) a(1), lickTimeAfterCueC(valRwdTrI), 'UniformOutput', false);
...
%    lat.rwdFstLatMissBlockMean = cell2mat(cellfun(@nanmean, rwdFstLatMissBlocks, 'UniformOutput', false));
    ...
    if sum(trPnsI)>5 % if there are No-Go trials
        ...
        lat.pnsFstLat(valPnsTrI) = cellfun(@(a) a(1), lickTimeAfterCueC(valPnsTrI), 'UniformOutput', false);
        
%         lat.pnsFstLatMiss = cell(1, length(tbytDat)); lat.pnsFstLatMiss(:) = {NaN};
%         lat.pnsFstLatMiss(valPnsTrI) = cellfun(@(a) a(1), lickTimeAfterCueC(valPnsTrI), 'UniformOutput', false);
