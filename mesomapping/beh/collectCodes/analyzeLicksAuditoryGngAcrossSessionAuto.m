...
%% prepare the figSaveDir
figSaveDir = fullfile('/Volumes/buschman/Rodent Data/dualImaging_parkj', 'collectFigure'); 
...
for f = 2:length(filePaths)    
    ...
    subFolders = find_keyword_folder_sortByDate(filePaths{f}, 'task'); 
    %% Main Loop
    for ff = 1:length(subFolders)
        ...
        lickFileC = find_keyword_file(fullfile(subFolders{ff}, 'Matfiles'), '_LickAnalysis', true);

        if ~isempty(lickFileC)
            load(fullfile(lickFileC{1}), 'rez', 'var');

            dPrmTot(f, ff) = rez.sigD.dprime;

            % collect dPrime data
            dPrmC{1, ff, f} = header;
            dPrmC{2, ff, f} = cell2mat(cellfun(@(a) a.dprime, rez.sigDBlocks, 'UniformOutput', false));

            % collect latency data
            latGoC{1, ff, f} = header;
            latGoC{2, ff, f} = rez.lat.rwdFstLatBlockMean;

            latNogoC{1, ff, f} = header;
            latNogoC{2, ff, f} = rez.lat.pnsFstLatBlockMean;
    ...
    %% plot
    % plot dPrime
    hfig = sigDrezDprmPlotAcrossSession('dPrime Across Session', dPrmC(2, :, f)', blueShades);
    print(hfig, fullfile(figSaveDir, [mID, '_LickAnalysis_acrossSession_dPrime']), '-dpdf', '-vector');

    % plot first lick latency
    hLat = firstLatencyPlotGngAcrossSession('first lick latency across session', latGoC(2, :, f)', latNogoC(2, :, f)', cool);
