...
for f = 1:length(filePaths)
    ...
    filePathDatParseGng = GrabFiles_sort_trials('_tbytDat_parseGng', 0, {fullfile(filePaths{f}, 'Matfiles')});
    if ~isempty(filePathDatParseGng)
        load(filePathDatParseGng{1}, 'tbytDat')
    else
        filePathDat = GrabFiles_sort_trials('_tbytDat.mat', 0, {fullfile(filePaths{f}, 'Matfiles')});
        if ~isempty(filePathDat)
            load(filePathDat{1}, 'tbytDat')
            tbytDat = parseAuditoryGngTrials(tbytDat);
            save(fullfile(filePaths{f}, 'Matfiles', strcat(header, '_tbytDat_parseGng')), 'tbytDat')
        else
            error("Cannot locate the tbytDat file!")
        end
    end
    ...
    %% Analyze and plot
    if sum(punishTrI) >= 10  % Go-NoGo sessions with airpuffs
         ...
         hAxZ = plotMeanSemSubplot('Lick counts Go/NoGo (Hz)', rez.rezGngLickHz.mLickHz, rez.rezGngLickHz.sLickHz, ...
             var.rezGngLickHz.timeX_binnedLickHz, [2, length(filePaths), f], cool, {'Go', 'NoGo'});
         xlim(hAxZ, [-1 5]);
         ylim(hAxZ, [0 9]);
         xticks(hAxZ, -2:1:6); 
        ...
        hAxR = rasterPlotCellSubplot('Lick rasters Go/NoGo trials', rez.lickTimeGngC, [2, length(filePaths), f+length(filePaths)], cool, 0.8);
        xlim(hAxR, [-1 5]);
        xticks(hAxR, -2:1:6); 
        xtickangle(0);
    end
...
%% save and print
print(hfig, fullfile(fileparts(filePaths{1}), 'collectFigure', [mID, '_LickRastersAcrossSessions']), '-dpdf', '-vector');
