...
filePaths = {'/Volumes/buschman/Rodent Data/Behavioral_dynamics_cj/DA003/DA003_101523', ... % Day2
             '/Volumes/buschman/Rodent Data/Behavioral_dynamics_cj/DA008/DA008_101623', ... % Day3
...
% prepare colormaps
blueShades = generateColormap([176 226 255]./255, [0 0 128]./255, 200); % for tertiles
...
for f = 1:length(filePaths)
    ...
    % load tbytDat
    filePathDat = GrabFiles_sort_trials('_tbytDat', 0, {fullfile(filePaths{f}, 'Matfiles')});
    [~, header] = fileparts(filePaths{f});
    load(filePathDat{1}, 'tbytDat')

    % load stimInfo
    filePathStim = GrabFiles_sort_trials('_stimInfo', 0, {fullfile(filePaths{f}, 'Matfiles')});
    if ~isempty(filePathStim)
        load(filePathStim{1}, 'stimopts')
    else
        filePathStim = GrabFiles_sort_trials('_stimInfo', 0, filePaths(f));
        load(filePathStim{1}, 'stimopts', 'opts')
        [stimOptsFolder, stimOptsName] = fileparts(filePathStim);
        save(fullfile(stimOptsFolder, 'Matfiles', stimOptsName), 'stimopts', 'opts')
    end

    var.stimopts = stimopts; clearvars stimopts opts

    assert(length(tbytDat)==length(var.stimopts.rewarded_stim))

    %% prep variables and data
    var.trDur = round(nanmean(cell2mat(cellfun(@(a, b)  b-a, {tbytDat.stimOn}, {tbytDat.stimOff}, 'un', 0)))); % trial duration
    var.minMaxTpre = [-var.prePostWinSize 0];
    var.minMaxTcue = [0 var.trDur];
    var.minMaxTpost = [var.trDur var.trDur+var.prePostWinSize];

    % lick timestamps aligned to the cue onset
    rez.lickTimeC = cellfun(@(a, b) a-b, {tbytDat.Lick}, {tbytDat.stimOn}, 'un', 0);

    % divide trials into tertiles excluding first 10 and last 10 trials
    var.tertiles = divideTrials(length(tbytDat), 3, 10, 10);
    var.gngTrials = {find(var.stimopts.rewarded_stim), find(var.stimopts.punished_stim)}; % Go and NoGo trials

    %% (Skip for now) lick simulations tertiles
    %[rez.lickSimInC, rez.lickSimOutC, rez.lickCntC] = lickSimWrapper(var, rez);

    %% Analyze and plot
    if sum(var.stimopts.punished_stim) >= 10  % Go-NoGo sessions with airpuffs
        % z-scored lick counts
        var.rwdPnsTrials = cellfun(@logical, {var.stimopts.rewarded_stim, var.stimopts.punished_stim}, 'UniformOutput', false); % Go, No-Go trial indices
        [rez.zrezGng, var.zrezGng] = binnedLickZWrapper(var, rez.lickTimeC, var.rwdPnsTrials);
        % plot z-score normalized lick counts
        hAxZ = plotMeanSemSubplot('Lick counts Go/NoGo (z)', rez.zrezGng.mLickZ, rez.zrezGng.sLickZ, ...
            var.zrezGng.timeX_binnedLickZ, [2, 3, 1], cool, {'Go', 'NoGo'});

        % lick raster plot tertiles
        ...
        [rez.sigD] = signalDetectionLickAnalysis(tbytDat, var.stimopts.rewarded_stim, var.stimopts.punished_stim);

        rez.sigDTertiles = cell(1, length(var.tertiles));
        for tt = 1:length(var.tertiles)
            tI = var.tertiles{tt};
            rez.sigDTertiles{tt} = signalDetectionLickAnalysis(tbytDat(tI), var.stimopts.rewarded_stim(tI), var.stimopts.punished_stim(tI));
        end
        hAxD = sigDrezSubplot('SigD tertiles', rez.sigDTertiles, [2, 3, 2], pastels);
        hAxDp = sigDrezDprmSubplot('dPrime tertiles', rez.sigDTertiles, [2, 3, 5], pastels); 

        % first lick latency
        [rez.lat] = latencyOfFirstLicks(tbytDat, var.stimopts.rewarded_stim, var.stimopts.punished_stim, var.minMaxTcue);
        hAxL = blockMeanSubPlot('First lick latency', {rez.lat.rwdFstLatMissBlockMean, rez.lat.pnsFstLatMissBlockMean}, [2, 3, 3], cool, {'Go', 'NoGo'});
        xlabel(hAxL, 'Block');
        ylabel(hAxL, 'Latency (s)');
        

    else % training sessions without airpuffs
        % z-score normalization tertiles
        [rez.zrezTertiles, var.zrezTertiles] = binnedLickZWrapper(var, rez.lickTimeC, var.tertiles);
        % plot z-score normalized lick counts
        hAxZ = plotMeanSemSubplot('Lick counts tertiles (z)', rez.zrezTertiles.mLickZ, rez.zrezTertiles.sLickZ, ...
            var.zrezTertiles.timeX_binnedLickZ, [2, 2, 1], blueShades, {'Block 1', 'Block 2', 'Block 3'});

        % lick raster plot tertiles
        rez.lickTimeTertileC = cellfun(@(a) rez.lickTimeC(a), var.tertiles, 'UniformOutput', false);
        hAxR = rasterPlotCellSubplot('Lick rasters tertiles', rez.lickTimeTertileC, [2, 2, 3], blueShades, 0.8);
        ...
        hAxD = sigDrezSubplot('SigD tertiles', rez.sigDTertiles, [2, 2, 2], pastels);
    ...
    %% save and print
    if exist(fullfile(filePaths{f}, 'Figure'), 'dir')==0
        mkdir(fullfile(filePaths{f}, 'Figure'))
    end

    print(hfig, fullfile(filePaths{f}, 'Figure', [header, '_LickAnalysis']), '-dpdf', '-vector');

    save(fullfile(filePaths{f}, 'Matfiles', [header, '_LickAnalysis']), 'rez', 'var');
