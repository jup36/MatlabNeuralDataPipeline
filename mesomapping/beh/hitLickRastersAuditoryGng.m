function [hitLickLatencyC, waterLatencyC] = hitLickRastersAuditoryGng(filePath)
...
% evtOn-aligned lick rasters
hitLickLatencyC = cellfun(@(a, b) a-b, {tbytDat(hitI).Lick}, {tbytDat(hitI).evtOn}, 'UniformOutput', false); 
waterLatencyC = cellfun(@(a, b) a-b, {tbytDat(hitI).water}, {tbytDat(hitI).evtOn}, 'UniformOutput', false); 
