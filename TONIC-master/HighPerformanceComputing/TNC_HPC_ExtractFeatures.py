...
def feature_extraction_command_line_parser(parser):
    ...
    parser.add_option("-C", "--compile_all",   action="store_true", dest="compile_all", help="compile all feat. extr. Matlab code", metavar="compile_all", default=False)
    ...
    parser.add_option("-F", "--filtered_data", action="store_true", dest="filtered_data", help="data is not raw, so don't perform fiiltering", default=False)
    ...
    parser.add_option("-r", "--snr",       dest="snr", help="threshold value of signal-to-noise ratio", metavar="snr", default="0");
    ...
    parser.add_option("-S", "--slots",dest="num_slots",help="# of cluster slots per 1 job (default=2)", metavar="num_slots", default=2)
    parser.add_option("-t", "--array_type",dest="array_type",  help="electrode array type, e.g. NN_b64, NN_b32, NN_b64_dhs,  NN_b32_dhs, tetrode, etc.", metavar="electrode array type", default="")
    parser.add_option("-T", "--num_segments",   dest="num_segments", help="# of time segments (default=1)", metavar="num_segments", default=1) 
...
def compile_code(options):
    print "\nCompiling TNC_SSP_ExtractFeatures.m ..."
    input_path = os.path.join(tonic_home,"EventDetectionClassification", \
                              "TNC_SSP_ExtractFeatures.m")
    command = "mcc -m -N -p signal -p optim -p stats -p shared -I /home/denisovg/software/drtoolbox -R -singleCompThread " +\
    ...
    if options.compile_all:
        ...
        if options.verbose:
            ...
            print "command= ", command, "\n"
        os.system(command)
        os.system("chmod +x " + os.path.join(os.path.join(tonic_home, "Bin", "TNC_HPC_MergeMatFiles")))
...
def create_high_level_shell_script(outfolderpath, input_file_path, options):
    ...
    for i in range(1, int(options.num_segments)+1):
        ...
        shell_script_command += " " + input_file_path[0]\
                             ...
                             +  " -r " + options.snr \
                             +  " -t " + options.array_type \
                             +  " -n " + options.num_electrodes
        if options.verbose:
            shell_script_command += " -v "
        if options.scm:
            shell_script_command += " -M "
        if options.filtered_data:
            shell_script_command += " -F "
...
def create_array_job_script(outfolderpath, input_file_path, options):
    ...
    shell_script_path = os.path.join(outfolderpath, \
                        "Extract_features." + input_file + ".sh")
    executable_path = os.path.join(tonic_home, "Bin", options.executable_name)
    if re.search("NN_b64", options.array_type):
        num_shanks = 8
    elif re.search("NN_b32", options.array_type):
        num_shanks = 4
...
def submit_array_job(shell_script_path, epilog_script_path, options):
    ...
    if re.search("qsub", options.submission_command):
        ...
        command += " -pe batch " + str(options.num_slots) + " -l sandy=true "
        ...
        if options.verbose:
            print "Submit feature extraction job command=", command, \
                  "\narray_jobid=", array_jobid, " res=",
