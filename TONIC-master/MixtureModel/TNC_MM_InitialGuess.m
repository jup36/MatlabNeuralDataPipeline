...
function [PI, MU, SIGMA] = TNC_MM_InitialGuess(DATA, iter, options)
    %
    disp(' ');
    ...
    disp(['options.guess_method=' options.guess_method ]);

    if strcmp(options.guess_method, 'kmeans')     
        disp('Entered TNC_MM_InitialGuess ...');
        ...
        [PI, MU, SIGMA] = get_kmeans_initial_guess(DATA, iter, options);
    else         
        disp('Computing randomized initial guess...'); 
        [PI, MU, SIGMA] = get_random_cell_initial_guess(DATA, iter, options);
    end
...
function [PI, MU, SIGMA] = get_kmeans_initial_guess(DATA, iter, options)
    disp(['options.M=' num2str(options.M)]);
    M = options.M;
    K = size(DATA, 2);
    ...
    for i=1:(iter*int16(max(options.iRepl, 1)))
        r = rand(1,M);  % M-vector of random numbers between 0 and 1
    end
    cols = max(1, round(N*r));
    cols
    seeds = DATA(cols, :);
%   Y = kmeans(DATA, M, 'start', seeds,   'display','final');
%   Y = kmeans(DATA, M, 'replicates', 100, 'display','final');
    best_min_counts = 1;
    best_counts = zeros(1, M);
    Y_best = [];
    for i=1:100
        try
            Y = kmeans(DATA, M);
...
%       disp(['i=' num2str(i) ' counts=' num2str(counts)]); 
        if best_min_counts <= min(counts) 
           best_min_counts = min(counts);   
           best_counts = counts; 
           Y_best = Y;
           Y_best_unique = unique(Y');
        end
        if min(counts) > 1
            break;
        end
...
%   disp(['Y=' num2str(Y')]); 
    if options.verbose
        disp(['Y_unique=', num2str(Y_best_unique) ' counts=' num2str(counts)]);
        disp(' ');
    end

    PI        = ones(1, M) * 1./double(M);
    MU        = zeros(K, M);
    SIGMA     = zeros(K, K*M);
    SIGMA_DET = zeros(1, M);
    for m= 1:M
        ...
        disp(' ');
        disp(['m=' num2str(m) ' SIGMA_DET=' num2str(SIGMA_DET(m))]);
    end
    ...
    for m=1:M
        ...
        elseif K == 8
            ...
            [XC2, YC2, ZC2] = TNC_MM_Triangulation(MU([2 4 6 8],m)', tetrode1', options);
            ...
            if options.verbose >= 2
                disp('TNC_MM_InitialGuess: K must be a multiple of 4');
            end
            return
        end
    end
...
function [ PI, MU, SIGMA] = get_random_cell_initial_guess(DATA, iter, options)
    M = options.M;
    N = size(DATA, 1);
    K = size(DATA, 2);
    PI    = zeros(1, M);
    ...
    if K == 8
        [XE, YE, ZE]  = TNC_MM_ElectrodeCoordinates('Buzsaki64', (1:8));
    else
        [XE, YE, ZE]  = TNC_MM_ElectrodeCoordinates('tetrode', (1:4));
    end
...
%       disp(['Xn=' num2str(Xn) ' distn=' num2str(distn)]); 
    ...
    % Determine PI, MU and SIGMA 
    for m=1:M
        IND = find(Y == m);
        if length(IND) > 0
            DATA1 = DATA(IND, :);
            PI(m) = double(size(DATA1,1))/double(length(Y));
            MU(:,m) = mean(DATA1);
