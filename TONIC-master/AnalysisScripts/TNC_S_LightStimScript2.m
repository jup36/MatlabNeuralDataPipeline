...
if currParams.filter.causal
    ...
    offset = (numel(currParams.filter.kernel)./2);
else
        ...
        if numel(allStimInds)==0
            ...
            for j = 1:numUnits
                ...
                if PopData.session(i).unit(j).ts(1) < PopData.session(i).stim(p).ts(1) & PopData.session(i).unit(j).ts(numStamps)>PopData.session(i).stim(p).ts(numel(allStimInds))
                    ...
                    [rsLITE] = TNC_AlignRasters(tmpSmooth , PopData.session(i).unit(j).ts , -1 , PopData.session(i).stim(p).ts , [PopData.currParams.winParams.prior ,PopData.currParams.winParams.after],0,1);
...
%         subplot(2,2,j); 
        ...
        baseline(j,i) = mean(PopData.session(i).unit(uI).stim(whichStim).psthAVG(1:currParams.winParams.prior).*1000);
    end    
...
for p=1:6
    ...
    for k=1:numUnits
        if lightALIGN.stim(p).dur>10
            lightALIGN.stim(p).respSyn(k) = trapz(lightALIGN.stim(p).psthZ(currParams.winParams.prior+5:currParams.winParams.prior+lightALIGN.stim(p).dur,k));
        else
...
%     figure(1); subplot(6,1,p); hist(lightALIGN.stim(p).respDir,-40:1:40);
    ...
    colormap(mapName);
    axis([-150 250 1 numUnits]);
    
    figure(201);
    if p==1
        clf;
    end
    subplot(1,6,p); plot(-500:500,mean(lightALIGN.stim(p).psthZ(:,lightALIGN.stim(p).sortSyn(quartInhib)),2),'Color',[0 0.67 1]); hold on;
