function [tonicDataStructure] = TNC_PhotoStimToStruct(dataStructure,unitArrayToLoad,toneID,liteID)
...
lost = 1;
tonicDataStructure.events.LITE.ts     = 0;
tonicDataStructure.events.TONE.ts     = 0;

% analog events
tonicDataStructure.events.LITE.ts     = dataStructure.neurons{liteID}.timestamps.*1000;
tonicDataStructure.events.TONE.ts     = dataStructure.neurons{toneID}.timestamps.*1000;

disp([ 'Number of light stimulation trials: '   num2str(length(tonicDataStructure.events.LITE.ts)) ]);
disp([ 'Number of tone stimulation trials: '    num2str(length(tonicDataStructure.events.TONE.ts)) ]);
