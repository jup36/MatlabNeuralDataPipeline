...
function [features] = TNC_SSPL_EventQuantSE(rawEvents,template)
...
%% Get scalar quantification
    ...
    for j=1:1:numEvents
        ...
        forPCA(j,:) = single(rawEvents.wfs(j).values(1,:));
        
        sclFeats(j,1) = min(single(rawEvents.wfs(j).values(1,15:17)));                             % min value
        sclFeats(j,2) = max(single(rawEvents.wfs(j).values(1,1:31)));                             % max value
%         sclFeats(j,3) = trapz(rawEvents.wfs(j).values(1,posInds)) - trapz(rawEvents.wfs(j).values(1,negInds)); % peak-valley area
        sclFeats(j,3) = trapz(single(rawEvents.wfs(j).values(1,16-7:16+7))); % peak-valley area
        sclFeats(j,4) = sclFeats(j,2) - sclFeats(j,1);                       % peak-valley amplitude
        sclFeats(j,5) = find(single(rawEvents.wfs(j).values(1,:))==min(single(rawEvents.wfs(j).values(1,:))),1);           % time of peak
        sclFeats(j,6) = numel(negInds);                                      % valley width
        sclFeats(j,7) = numel(posInds);                                      % peak width
        sclFeats(j,8) = trapz(abs(single(rawEvents.wfs(j).values(1,:))));                      % energy
        sclFeats(j,9) = sum(single(rawEvents.wfs(j).values(1,:).^2));                          % nonlinear energy
        sclFeats(j,10) = median(abs(diff(rawEvents.wfs(j).values(1,:))));              % median derivative
%         p = polyfit(0:29,single(rawEvents.wfs(j).values(1,16:numSamps)),2);              % fit a cruve to spike recovery
%         sclFeats(j,10) = p(2); % take the linear term
