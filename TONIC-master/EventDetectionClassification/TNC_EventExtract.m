...
% events.numEvents
    ...
    waveforms = zeros(numberOfEvents,windowPnts(1,2)+windowPnts(1,1)+1,'single');
...
%% Check to make sure there is enough room to extract a window
    ...
    for i=1:numberOfEvents
        ...
        if candidateEvents(1,i)-windowPnts(1,1) < 1
            ...
            events.wfs(i).values(1,:) = single( [zeros(1,pad+1) , bandPassedData(1,1:candidateEvents(1,i)+windowPnts(1,2))] );
%             events.raw(i).values(1,:) = single( [zeros(1,pad+1) , rawData(1,1:candidateEvents(1,i)+windowPnts(1,2))] );            
        ...
        elseif candidateEvents(1,i)+windowPnts(1,2) > numSamples
            ...
            events.wfs(i).values(1,:) = single( [bandPassedData(1,candidateEvents(1,i)-windowPnts(1,1):numSamples) , zeros(1,pad)] );
%             events.raw(i).values(1,:) = single( [rawData(1,candidateEvents(1,i)-windowPnts(1,1):numSamples) , zeros(1,pad)] );
        
        else
            
            events.wfs(i).values(1,:) = single( bandPassedData(1,candidateEvents(1,i)-windowPnts(1,1):candidateEvents(1,i)+windowPnts(1,2)) );
