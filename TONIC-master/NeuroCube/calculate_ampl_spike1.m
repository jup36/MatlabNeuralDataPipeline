...
function VoltageData = calculate_ampl_spike1(cellName, trial, start_end_times, sigma, xyMax, Z, gridSize, dist, Par_sim)
Par_sim
...
if Par_sim.point_source > 0
    ind1_max_diameter = find(start_diams == max(start_diams));
    ind2_max_diameter = find(  end_diams == max(  end_diams));
    ind1_max_diameter
    ind2_max_diameter
    disp(['ind1_max_diameter=' num2str(ind1_max_diameter) ' ind2_max_diameter=' num2str(ind2_max_diameter)]);
else
    ind1_max_diameter = [];
    ind2_max_diameter = [];
end
...
disp(['size(pt_coord)=' num2str(size(pt_coord)) ' size(start_segs)=' num2str(size(start_segs)) ' size(end_segs)=' num2str(size(end_segs)) ' size(h)=' num2str(size(h)) ' size(ds)=' num2str(size(ds)) ' size(R)=' num2str(size(R)) ' length(calc_times)=' num2str(length(calc_times))]);
disp(['dist=' num2str(dist) ' ds max, min=' num2str([max(ds)  min(ds)]) ' h max, min=' num2str([max(max(h))  min(max(h))]) ' R max, min=' num2str([max(max(R))  min(max(R))])]);
...
%       disp(['size(I_segs)=' num2str(size(I_segs))]);
	[Voltage_segs]= get_phi1(h,R,ds,I_segs,sigma, pt_coord, dist, ind1_max_diameter, ind2_max_diameter,Par_sim);
