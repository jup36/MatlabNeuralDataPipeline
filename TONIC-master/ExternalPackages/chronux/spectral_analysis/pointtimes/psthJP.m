function [R,t,E] = psthJP(data,sig,plt,T,err,t)
...
% average but is time dependent. Regions rich     
...
% psth (/Users/parkj/Documents/MATLAB/TONIC-master/ExternalPackages/chronux/spectral_analysis/pointtimes)
% just commented by JP Aug/2017
...
if nargin < 2; sig = 0.05;end  % the default std dev of Gaussian to be 50 ms
...
if nargin < 4 % in case T (time interval) is not specified, default is all
  T(1) = min(data(:,1));
  T(2) = max(max(data));
else    % in case T (time interval) is specified
  T(1) = T(1)-4*sig;
  T(2) = T(2)+4*sig;
end
