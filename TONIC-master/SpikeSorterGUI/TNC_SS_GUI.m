...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
if nargin && ischar(varargin{1}) && numel(findstr('.mat',varargin{1})) == 0
    gui_State.gui_Callback = str2func(varargin{1});
end

if nargin > 0 && length(findstr('_ft.mat', varargin{1})) > 0
    my_fileName = varargin{1};
end
...
function selectData_CreateFcn(hObject, eventdata, handles)
    ...
    if ~exist(my_fileName, 'file')
        disp(['NOTE: input file ' my_fileName ' is not found']); 
    end

    if numel(d)>0
        for i=1:numel(d)
            listOfFiles{i} = d(i).name;
            if length(findstr(char(my_fileName), char(listOfFiles{i}))) > 0
               temp = listOfFiles{1};
               listOfFiles{1} = char(my_fileName);          
               listOfFiles{i} = temp;
            end
        end
        handles.fileName = listOfFiles{1};
...
function loadData_Callback(hObject, eventdata, handles)
    ...
    gm_fileName = strcat(handles.fileName(1:(length(handles.fileName)-7)), '_gm.mat');
    try
        G = load(gm_fileName);
        gmStruct = G.gmStruct;
    catch 
    ...
    forSort = TNC_SS_CreateSortStruct(S.featStruct, gmStruct);
...
function manClustAdd1_Callback(hObject, eventdata, handles)
    ...
    try
        newId = handles.addId;
        [newClustIds1] = TNC_SS_AddCluster(handles,newId);
%       handles.featureData.seg(handles.segList(1)).shank(handles.shankNum).id = newClustIds1;
...
% handles    structure with handles and user data (see GUIDATA)
    ...
    if strcmp(handles.clusteringMode,'mixmodel');
        handles = TNC_SS_StoreAutoClustIds(handles);
    end
...
function kMeansButton1_Callback(hObject, eventdata, handles)
    ...
    try 
        ...
        disp(['Unable to run k-means with ' num2str(handles.numClusts(1)) ...
              ' clusters. Please, try a different number of clusters']);
    end
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
    if strcmp(handles.clusteringMode,'kmeans')
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
    handles.clusteringMode='mixmodel';
    handles = TNC_SS_StoreAutoClustIds(handles);
