function [ reachStart, reachStop, reachMW, pos1, pos2, xpos1, ypos1, xpos2, ypos2 ] = getReachTimesByPosition( positionData, TbetweenR, maxDur, cutOffPerc )
%Extracts best reach start times based on joystick position (deflection)
...
%TbetweenR = 1000; % minimum time between reaches, in ms
...
cutoff  = sortPos(round(length(sortPos)*cutOffPerc));    % set the 90% cutoff for the position data
cutoffd = sortVel(round(length(sortVel)*cutOffPerc));    % set the 98% cutoff for the velocity data (velocity cutoff is required, since reaches are expected to occur at high velocity)
allValSamps1=find(reachMW>cutoff);   % velocities exceeding the cutoff
allValSamps1=allValSamps1(allValSamps1<(length(reachSG)-300)); % this line is just to exclude the data points at the tail
allValidSamps=[];

for jj = 1:length(allValSamps1) % all points exceeding the pos cutoff
    if isempty(allValidSamps)   % in case of the 1st pos data point
        if sum(reachSG(allValSamps1(jj):allValSamps1(jj)+200)>cutoff)>20 % sort out high-pos reaches from the short jerks which is not really a reach
    ...
    else    % from the 2nd high-pos data point on 
        if (allValSamps1(jj)-allValSamps1(jj-1))>100 % if the current high-pos point came 100 ms or more after the previous high-pos point; this will prevent the redundant counting from a continued reach
...
%% detect reach start
reachStart=[]; % the goal here is to find the near-zero point that is immediately prior to each high-pos point  
for i  = 1:length(allValidSamps)
    currSamp = allValidSamps(i); % current valid sample (high-pos data point) 
    ...
    if nearZero>10
        ...
        realStartMinus10=currSamp; % take the current valid sample (high-pos data point) as a reach start point
    end
