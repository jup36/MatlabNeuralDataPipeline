function [ reachStart, noReachTime, reachMW ] = getReachTimesDecode( positionData )
...
cutoff  = sortPos(round(length(sortPos)*.50)); %90));    % set the 90% cutoff for the position data
cutoffd = sortVel(round(length(sortVel)*.70)); %98));    % set the 98% cutoff for the velocity data (velocity cutoff is required, since reaches are expected to occur at high velocity)
...
reachStart = reachStart(reachStart>5000 & reachStart<length(reachMW)-5000); % cut reachStart

%% detect timepoints without and far from detected reaches
tmpNoReach = find(reachMW<.02);
noReachFun = @(t) min(abs(t-reachStart)); % min distance to closest reachStart
noReach = tmpNoReach(cell2mat(arrayfun(noReachFun, tmpNoReach, 'Un', 0))>2000); 

noReachB = zeros(length(noReach),2); 
binChange = 1; 
noReachM = []; 
for no = 1:length(noReach)
    noReachB(no,2) = noReach(no); 
    if no ==1
        noReachB(no,1) = binChange; 
    elseif noReach(no)-noReach(no-1)<1000 
        noReachB(no,1) = binChange; 
    elseif noReach(no)-noReach(no-1)>1000 
        noReachM = [noReachM; nanmedian(noReachB(noReachB(:,1)==binChange,2))]; 
        binChange = binChange +1;        
        noReachB(no,1) = binChange; 
    end
end

noReachTime = noReachM(noReachM>5000 & noReachM<length(reachMW)-5000); 
noReachTime = round(noReachTime'); 
figure; plot(reachMW); hold on; plot(noReachM,repmat(0.005, 1, length(noReachM)), 'or')
plot(reachStart, repmat(.01, length(reachStart)), '*b')
