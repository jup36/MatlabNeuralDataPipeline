...
reachStart = reachStart(reachStart>5000 & reachStart<length(reachMW)-5000);
%dcReachStart = decimate(reachMW,1000); 
tmpNoReach = find(reachMW<.01);
noReachFun = @(t) min(abs(t-reachStart)); % min distance to closest reachStart
noReach = tmpNoReach(cell2mat(arrayfun(noReachFun, tmpNoReach, 'Un', 0))>5000); 
