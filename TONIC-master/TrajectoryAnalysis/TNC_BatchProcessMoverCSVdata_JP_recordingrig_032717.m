fileDirectory = '/Volumes/RAID/parkj/BehavioralData/367796 #5/032717/'; 
cd(fileDirectory)
...
for i = 1:size(allFiles,1)
    ...
    reachTriStartIndex2 = find(behavStateDiff == 2 & data.trajectories.contXY(1:end-1,5) == 0); % this should detect the shift of paradigmmode from 0 to 2 which indicates start of the trial 
    %reachTriStartIndex3 = find(behavStateDiff == 3); % no such case
    reachTriStartIndex  = reachTriStartIndex2; %[reachTriStartIndex2; reachTriStartIndex3];
    reachTriStartIndex  = sort(reachTriStartIndex);
    contStartTimes      = data.trajectories.contXY(reachTriStartIndex+1,1);  % find the start time of trajectories in the continuous file
    
    % this part simply matches the # of trial and reach
    if nnz(contStartTimes) ~= nnz(data.trialTimes(:,2)) % nnz counts the number of nonzero elements in the matrix, ~= tests inequality
    ...
    if exist(fileNameStr,'file')== 1 || exist(fileNameStr,'file')== 2
        ...
        data.trajectories.contXYsmooth(beginTrial:end,2) = tan(data.trajectories.contXYsmooth(beginTrial:end,2).*(.1*(pi./180)))*90; % my joystick is 90 mm long, .1 (what is this scalar for? must be mm to cm conversion), pi./180 (degree to radian conversion)
        ...
        %data.trajectories.contXYsmooth(:,4)  = data.trajectories.contXY(:,5);  % behavioral state
        %data.trajectories.contXYsmooth(:,5)  = data.trajectories.contXY(:,4);  % licking (void in training data)
        ...
        data.trajectories.sLeverVm = medfilt1(data.trajectories.contXYsmooth(:,5),5);  % medfilt1(data.trajectories.contXYsmooth(:,5),5); % smooth velocity
