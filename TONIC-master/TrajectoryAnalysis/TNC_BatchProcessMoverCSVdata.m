...
behaviorIndex = strfind(fileDirectory, 'Head Fixed');
slashesIndex  = strfind(fileDirectory, '/');
slashPostBehav = find(slashesIndex > behaviorIndex, 1, 'first');
mousetype  = fileDirectory(slashesIndex(slashPostBehav)+1: slashesIndex(slashPostBehav+1)-1);      
...
for i = 1:size(allFiles,1)
    ...
    fileBase       = allFiles(i).name(1:max(strfind(allFiles(i).name,'_')-1));
    year           = allFiles(i).name(strfind(allFiles(i).name, '201'):strfind(allFiles(i).name, '201')+3);
...
%% Script to load csv behavior data
    ...
    data.trialTimes = dlmread(fileNameStr,',',2+skipInitTrials,0);
    ...
    % data.trajectories.contXY format
        ...
        % data.trajectories.contXY (:,5) = time when stimulation is occuring (formerly column 5)
        % data.trajectories.contXY (:,6) = 'behavioral state'- in iti, reward delay, motor, etc.
...
%% General information about the file
  ...
  % program starts with ITI- need to remove that with a shift in order to compare continuous data and trial times file
    behavStateDiff      = diff(data.trajectories.contXY(:,6));
    
    reachTriStartIndex2 = find(behavStateDiff == 2);
    ...
    if nnz(contStartTimes) ~= nnz(data.trialTimes(:,2))
        
       contLarge = nnz(contStartTimes) > nnz(data.trialTimes(:,2));
       contSmall = nnz(contStartTimes) < nnz(data.trialTimes(:,2));
       if contLarge == 1
           contStartTimes(nnz(data.trialTimes(:,2))+1:end) = [];
       end
       
    end
       
    shift       = contStartTimes - data.trialTimes(:,2);
...
%         data.trajectories.contXYsmooth(beginTrial:end,3) = tan(data.trajectories.contXYsmooth(beginTrial:end,3).*(.1*(pi./180)))*105;
 ...
 % Fill in Associated Kinematics
        ...
        data.trajectories.sLeverVm = medfilt1(data.trajectories.contXYsmooth(:,5),5); % smooth velocity
