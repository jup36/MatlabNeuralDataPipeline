function [sessAnalysis] = TNC_OpenFieldReportJustTrack(dataStructure,figNum,range)
...
if range==0
    range(1) = 100;
    range(2) = numel(dataStructure.data.posSmth.Xrmed);
end
...
currW       = dataStructure.data.posRaw.w(range(1):range(2));
currH       = dataStructure.data.posRaw.h(range(1):range(2));

%% Main figure for output display
currFileName = [num2str(dataStructure.data.year) '.' num2str(dataStructure.data.month) '.' num2str(dataStructure.data.day) ' | ' num2str(dataStructure.data.hour) ':' num2str(dataStructure.data.min)];
h = figure(figNum);
clf;
...
grid on; view([-35 5]); axis([100 450 50 350 min(currStamps) max(currStamps)]); 
xlabel('X (px)');
ylabel('Y (px)');
zlabel('Time (samples)');
...
axis([100 450 50 350 min(currStamps) max(currStamps)]);
xlabel('X (px)');
ylabel('Y (px)');
zlabel('Time (samples)');
...
for i=1:numProgs
    ...
    xV = diff(currX(currStart:currStop));
    yV = diff(currY(currStart:currStop));
    
    vel = sqrt(xV.^2 + yV.^2);
    
    progLengths.maxV(i) = max(vel);
