        ...
        for p=2:numel(clustNums)                        
            ...
            if handles.avgWaves==1
                ...
                for qq=1:numel(currInds)
                    ...
                    temp_segList(i)
                    currInds(qq)
                    temp_wf = temp_wf + handles.sessionStruct.seg(temp_segList(i)).shank(handles.shankNum).wfs(currInds(qq)).values(:,:);
                end
                for rr=1:numSites
                    yOff = rr * 500;
                ...
                for qq=1:numel(currInds)
                    for rr=1:numSites
                        yOff = rr * 1000;
                        plot(xOff , handles.sessionStruct.seg(temp_segList(i)).shank(handles.shankNum).wfs(currInds(qq)).values(rr,:) + yOff ,'Color' , handles.cMap(clustNums(p),:))+(rand(1,3)); hold on;                    
                    end
                end
            end
        end
    ...
    if max(clustNums)>0 && handles.showXCorr==1
        ...
        for p=2:numel(clustNums)                        
            ...
            for qq=2:numel(clustNums)
                ...
                plot(TNCxcorr_x+ ((p-2)*55), (TNCxcorr_y ./ (max(TNCxcorr_y) - min(TNCxcorr_y))) - ((qq-2).*2),'Color', handles.cMap(clustNums(p),:)); hold on; %  
            end
        end
