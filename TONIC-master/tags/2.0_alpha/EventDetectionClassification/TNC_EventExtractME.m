function [events] = TNC_EventExtractME(bandPassedData,eventInds,windowPnts)
...
%           disp(['i=' num2str(i) ' eventInds=' num2str(eventInds(:,i)') ]);
%             mean_ind = round(mean(double(eventInds(1:numChan,i))));
            mean_ind = double(eventInds(i));
            wfs(i).values(1:numChan,:) = bandPassedData(1:numChan,mean_ind-windowPnts(1,1):mean_ind+windowPnts(1,2));
        end
    end
