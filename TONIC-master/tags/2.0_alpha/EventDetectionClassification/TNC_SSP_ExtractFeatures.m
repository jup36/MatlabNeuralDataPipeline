...
%% PRE-PROCESSING
    ...
    if ~isempty(strfind(dataFileName, '.ns'))
        try
           ...
           chunk   = floor(double(sigDATA.MetaTags.Duration) ./ options.numSegs);
    ...
    if options.verbose
        disp(' ');
        disp(['file_name=' dataFileName ' size(sigDATA)=' num2str(size(sigDATA.Data))]);
    end

    disp(' ');
    disp(' ');
    disp(['Data will be loaded and processed as ' num2str(uint32(options.numSegs)) ' x ' num2str(uint32(chunk)) ' seconds long segments.']);
    disp(' ');
    disp(' ');

%% PROCESS OPTIONAL INPUT PARAMETERS 
    strcmp(options.arrayType,'NN_b64')
    if strcmp(options.arrayType,'NN_b64')
        if channelCount ~= 64
    ...
    if options.inode > 0 
        ...
        my_j = mod(options.inode, y_dim);
        if my_j == 0
            my_j = y_dim;
        end
        if options.inode > y_dim
            my_k = (options.inode - my_j)/y_dim + 1;
        else
            my_k = 1;
        end
        ...
        if options.verbose
            disp(' ');
            disp(['y_dim=' num2str(y_dim) ' my_j=' num2str(my_j) ...
...
% LOAD BEHAVIORAL DATA TO DISPLAY WITH THE SPIKING DATA
    ...
    for k=min_k:max_k % loop through time segments 
        ...
        disp(['arrayType=' options.arrayType ' k=' num2str(k)]);
        ...
        snrThresh  =  6.5; % the lower this value, the more events will be detected
        if options.snr > 0
        ...
        switch options.arrayType
            ...
            case 'NN_b64' 
                ...
                process_multichannel_data(dataFileName,sigDATA,k,...
                                          ...
                                          sampleRate,snrThresh,winL,winR,options);
...
function [pcaStruct,featStruct,sessionStruct] = ...
    ...
    disp(['min_j=' num2str(min_j) ' max_j=' num2str(max_j)]);
    for j=min_j:max_j % loop through shanks
        ...
        for i=min_i:max_i % loop through channels in a given shank
            if numel( find(electrodeMatrix(i,j) == allRecChan) ) > 0
                % Filter only non-tetrrode data: the tetroder data are already filtered
                if ~strcmp(options.arrayType, 'tetrode') || options.rawData
...
%                       disp(['   rawData=' num2str(rawData)]);
                    ...
                    [~,hiBandData] = TNC_FilterData(rawData,sigDATA.MetaTags.SamplingFreq,0,0,[1 0]);
                    spkData(i,:) = hiBandData.values;   
                else
                    spkData(i,:) = sigDATA.Data(i,:);
                end                                             
            else
...
%           disp(['numel(find(allRecChan==electrodeMatrix(i,j)))=' num2str(numel(find(allRecChan==electrodeMatrix(i,j))))]);
            ...
            if numel(find(allRecChan==electrodeMatrix(i,j)))==1
                ...
                [spkEvents] = TNC_EventDetect(spkData(i,:),sampleRate,snrThresh);
            else
    ...
    % GENNADY'S CODE
        ...
        [ eventInds ] = find_events_across_shank(SortedSpikes, spkData);
    ...
    % CALCULATE FEATURES FOR SPIKE EVENTS                
        ...
        if k==1 || k==my_k
            [features] = TNC_EventQuantME(spkData, spikes.wfs, [], spikes.inds,...
                                          sampleRate, winL, shankSize);
            pcaStruct  = features.pca;
        else
            [features] = TNC_EventQuantME(spkData, spikes.wfs, pcaStruct, spikes.inds,...
                                          sampleRate, winL, shankSize);
        end
        disp('...done.');
        disp(' ');
    ...
    % UPDATE STRUCTURE FOR SPIKE SORTING
        ...
        disp('Updating the feature structure with new data...');
            ...
            featStruct.seg(k).shank(j).ts     = round(spikes.inds./sampleRate);
...
function [featStruct,sessionStruct] = ...
         process_isolated_site_data(FileName, sigDATA, k, min_k,  ...
                                    featStruct,sessionStruct,...
                                    numChan,my_j,my_k,chunk,sampleRate,...
    ...
    for eInd=min_eInd:max_eInd % shanks | sites
        ...
        disp('2) Detecting and extracting events...');
            ...
            % Detect indices of the signal minima corresponding to spikes  
            [spkEvents] = TNC_EventDetect(spkData,sampleRate,snrThresh);

            if numel(spkEvents.inds)>10
...
function [ eventInds ] = find_events_across_shank(Spikes, data)
    N = numel(Spikes);
    ...
    for i=1:(numel(Spikes)-1)
        ...
        if spikeBeg > 0 && (Spikes(i+1).position - Spikes(i).position > spikeSpacing)
            spikeEnd = i;
            spikeVecInd = spikeVecInd + 1;
            spikeVec(spikeVecInd).spikePosition = round((Spikes(spikeBeg).position+Spikes(spikeEnd).position)/2);
            spikeVec(spikeVecInd).channelPos = -1*ones(1, numChannels);
            for j=spikeBeg:spikeEnd
                c = Spikes(i+1).channel;
                spikeVec(spikeVecInd).channelPos(c) = Spikes(j).position;
            end
            % Update spike spikeVector
            for c=1:numChannels
                if spikeVec(spikeVecInd).channelPos(c) < 0
                    ...
                    if length(ind_pos_found) > 0  
                        ...
                        sbeg = max(1,             spikeVec(spikeVecInd).spikePosition - round(spikeSpacing/2));
                        send = min(numel(Spikes), spikeVec(spikeVecInd).spikePosition + round(spikeSpacing/2));
...
%           disp('spikeVec(spikeVecInd).channelPos=');
%           spikeVec(spikeVecInd).channelPos
            spikeBeg = -1;
        end
    end
