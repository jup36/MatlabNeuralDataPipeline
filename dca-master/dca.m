function [U, dcovs, p] = dca(Xs, varargin)
...
%           Each column of the loading matrix corresopnds to orthonormal basis projection vectors.  
%   dcovs: (1 x num_dca_dimensions vector), distance covariances for each
%           dimension identified by DCA
%
% EXAMPLE:
%   Xs{1} = randn(20,1000);  % think of it as neuron-by-trials matrix recorded in brain area 1 (e.g. 20 neurons)
%   Xs{2} = Xs{1}(1:5,:).^2; % think of it as neuron-by-trials matrix recorded in brain area 2 (e.g. 5 neurons) 
%   Ds{1} = squareform(pdist(Xs{1}(1,:)' + Xs{2}(1,:)')); % think of it as a trial-by-trial distance matrix of a dependent variable (distance across trials).  
%   [U, dcovs, p] = dca(Xs, Ds, 'num_dca_dimensions', 5, 'percent_increase_criterion', 0.1);
...
% addpath(genpath('/Volumes/RAID2/parkj/MATLAB'))
    ...
    %%% pre-processing
        p = parse_input(Xs, varargin);   % allows user to input name-value pairs, p = parse_input(Xs, Ds); 
        % p = parse_input(Xs, Ds);       % when running line-by-line make sure to
        % parse(p, Xs, Ds, 'num_dca_dimensions', 5, 'percent_increase_criterion', 0.1); % E.G. For some reason, when running line-by-line 'parse' needs to be run in this way because the 'parse' receives its input as 'vargs{:}', which obviously must be taken as 'parse(p, Xs, Ds ... )' when running line-by-line. 
    ...
    %%% optimization
        for idim = 1:num_dca_dims % increment the number of dca dimensions
            ...
            while (check_if_dcov_increases(p, total_dcov, total_dcov_old, itotal_dcov)) % continue on if dcov keeps increasing
                ...
                if (p.Results.num_stoch_batch_samples == 0)
                    ...
                    for iset = 1:num_datasets
                        ...
                        R_combined = get_recentered_combined(R((1:end)~=r(iset)), R_given);  % get combined recentered distance matrix (summed - except for the current recentered distance matrix of the current dataset), R_given contains D_given (see, preprocessing)
                                
                        % perform optimization for one dataset
                        u{r(iset)} = dca_one(X{r(iset)}, Xij{r(iset)}, R_combined, u{r(iset)}, col_indices, p);
                        
                        R{r(iset)} = get_recentered_matrix(u{r(iset)}, X{r(iset)}); % R should be sample x sample 
                    end
                ...
                if (idim ~= num_dca_dims)
                    for iset = 1:num_datasets
                        [Q,R] = qr([U{iset}(:,1:idim) ...
                                randn(size(U{iset},1), size(U{iset},1)-idim)]); % qr: orthogonal triangular decomposition
    ...
    function preprocessing(p)
        ...
        num_samples = size(p.Results.X{1},2); % added by JP to run line-by-line
    ...
    function initialization(p)
        ...
        for iset = 1:num_datasets
            if (~isempty(p.Results.u_0s) && size(p.Results.u_0s{iset},1) == size(X{iset},1)) % if user input initialized weights for first dim
                ...
                u{iset} = orth(randn(size(X{iset},1), 1)); % orthonormal basis for the range of a matrix
            end
        end

        %%% get initial recentered matrices for each dataset based on u
            if (p.Results.num_stoch_batch_samples == 0)  % only for full grad descent
                R = cell(1,num_datasets); % R is the re-centered distance matrix
                for iset = 1:num_datasets
                    R{iset} = get_recentered_matrix(u{iset}, X{iset}); % compute re-centered distance matrix of X projected onto u
                end
                
                total_dcov = get_total_dcov(R,D_given); % compute the total distance covariance across all datasets
...
function total_dcov = get_total_dcov(R,D_given)
    ...
    Rtotal = 0;
    T = size(R{1},1); % # of samples
    for iset = 1:length(R) % # of datasets
        for jset = (iset+1):length(R) 
            Rtotal = Rtotal + sqrt(1/T^2 * R{iset}(:)' * R{jset}(:)); % compute the distance covariance (a scalar)  
        end
    end

    total_dcov = Rtotal / ((length(R)-1)*length(R)/2); % normalize for the number of pairs
...
function R_combined = get_recentered_combined(R, R_given)
    ...
    if (~all(all(R_given==0))) % if R_given ~= 0, then D_given exists (see preprocessing)
    ...
    %%% OPTIMIZATION  
        for istep = 1:p.Results.num_iters_per_dataset  % stop when num iters have been reached
            ...
            % COMPUTE GRAD DESCENT PARAMETERS
                D_uXij = get_D_uXij(u);        % get distance matrix of current u (u'X)
                f_val = get_f(D_uXij);         % compute current function value (distance covariance between u'X and R_combined) and gradf for backtracking
