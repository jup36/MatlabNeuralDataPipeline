function [S,pc,sortedBinSpkCell] = pcaPSTHctxLabel( filePath, fileName, varName, varargin  )
...
%p = parse_input_psthPCA( filePath, fileName, varName, {'binSize',200,'stepSize',200,'binSizeZ',50,'useAllUnits',false,'PCsLogic',true,'PCs',5,'expVarLogic',false,'expVarCut',80,'FRcut',1,'nanTrialCut',.1,'cmap','hot','cAxis',[-3 6], 'rmvNaNtrials', true , 'rmvNaNunits', false} );
...
for f = 1:length(fileName) % increment files
    ...
    % get histOnt indices from probeTrackAnalysis
    hPath = dir('*_histOnt.mat'); 
    load(fullfile(hPath.folder,hPath.name),'tempMCtxIall'); 
    if ~length(tempMCtxIall)==length(S.SpkTimes)
        error('Cell # for histOnt and SpkTimes do NOT match!')
    end
    ...
    SvalFrCellId = zeros(size(S.binSpkZ,1),1); SvalFrCellId(S.valFrCellId)=true; 
    S.valFrCellId = find(SvalFrCellId&tempMCtxIall'); 
    
    [pc.loadings, pc.PCscore, pc.eigValues, ~, pc.expVar] = pca(S.binSpkZ(SvalFrCellId&tempMCtxIall',:)); % run pca on trial-averaged z scores (include all units passing the FR cut)
...
for f = 1:length(fileName)
    ...
    print( [fileName{f},'_trialAvgZscoreCtxLabel',varName],'-dpdf') 
    ...
    print( [fileName{f},'_trialAvgPsthPCsCtxLabel',varName],'-dpdf')
    
    %% Save variables
    cd(filePath)
    saveName = strcat(fileName{f},'pcaPSTHhistOnt_',varName,num2str(p.Results.binSize),'ms');
    S = file(f).S;
    pc = file(f).pc;
