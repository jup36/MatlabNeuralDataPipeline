function getHandTrjFromDlcData_learning(filePath, hTrjPath)
...
stereoCalibFile = '/Users/jp3025/Documents/TOOLBOX_calib/stereo_calib_Apr21/Calib_Results_stereo.mat'; % caltech camera calibration toolbox
...
csv_dup_I = cell2mat(cellfun(@(a) strcmpi(a(1:2), '._'), {allCsv.name}, 'un', 0));
allCsv(csv_dup_I) = []; 

%% get frame dimensions
allMP4 = dir(fullfile(hTrjPath,'*.mp4')); 
mp4_dup_I = cell2mat(cellfun(@(a) strcmpi(a(1:2), '._'), {allMP4.name}, 'un', 0));
allMP4(mp4_dup_I) = []; 

aVid = VideoReader(fullfile(allMP4(1).folder, allMP4(1).name)); % read the 1st front video to get frame info

wd = aVid.Width;  % frame width
ht = aVid.Height; % frame height
...
for f = 1:length(allCsv)
    ...
    tmpCsv = readcell(tmpTrj.FileName);
    tmpCsv = tmpCsv(2:end, :); % drop the 1st line
    
    if f == 1
        % locate variables in the csv file
        fg1Idx = cellfun(@(a) strcmpi(a,'Finger1'), tmpCsv(1,:));
        fg2Idx = cellfun(@(a) strcmpi(a,'Finger2'), tmpCsv(1,:));
        hdIdx = cellfun(@(a) strcmpi(a,'hand'), tmpCsv(1,:));
        jsTIdx = cellfun(@(a) strcmpi(a,'Joystick1'), tmpCsv(1,:));
        jsBIdx = cellfun(@(a) strcmpi(a,'Joystick2'), tmpCsv(1,:));
        
        xIdx = cellfun(@(a) strcmpi(a,'x'), tmpCsv(2,:));
        yIdx = cellfun(@(a) strcmpi(a,'y'), tmpCsv(2,:));
        lIdx = cellfun(@(a) strcmpi(a,'likelihood'), tmpCsv(2,:));
    end
    
    tmpTrj.fg1X = cell2mat(tmpCsv(3:end,fg1Idx & xIdx)); % finger1 x
    tmpTrj.fg1Y = cell2mat(tmpCsv(3:end,fg1Idx & yIdx)); % finger1 y
    tmpTrj.fg1lh = cell2mat(tmpCsv(3:end,fg1Idx & lIdx)); % finger1 likelihood
    ...
    tmpTrj.fg2X = cell2mat(tmpCsv(3:end,fg2Idx & xIdx)); % finger2 x
    tmpTrj.fg2Y = cell2mat(tmpCsv(3:end,fg2Idx & yIdx)); % finger2 y
    tmpTrj.fg2lh = cell2mat(tmpCsv(3:end,fg2Idx & lIdx)); % finger2 likelihood
    ...
    tmpTrj.hdX = cell2mat(tmpCsv(3:end,hdIdx & xIdx)); % hand x
    tmpTrj.hdY = cell2mat(tmpCsv(3:end,hdIdx & yIdx)); % hand y
    tmpTrj.hdlh = cell2mat(tmpCsv(3:end,hdIdx & lIdx)); % hand likelihood
    ...
    % jsT: joystick Top
    tmpTrj.jsTX = cell2mat(tmpCsv(3:end,jsTIdx & xIdx)); % finger1 x
    tmpTrj.jsTY = cell2mat(tmpCsv(3:end,jsTIdx & yIdx)); % finger1 y
    tmpTrj.jsTlh = cell2mat(tmpCsv(3:end,jsTIdx & lIdx)); % finger1 likelihood
    ...
    % jsB: joystick Bottom
    tmpTrj.jsBX = cell2mat(tmpCsv(3:end,jsBIdx & xIdx)); % finger1 x
    tmpTrj.jsBY = cell2mat(tmpCsv(3:end,jsBIdx & yIdx)); % finger1 y
    tmpTrj.jsBlh = cell2mat(tmpCsv(3:end,jsBIdx & lIdx)); % finger1 likelihood
...
%         [sPath,sName,~] = fileparts(sVideoInfo{t}.path);
        fPath = fVideoInfo{t}.path; 
        fName = fVideoInfo{t}.name; 
        sPath = sVideoInfo{t}.path; 
        sName = sVideoInfo{t}.name; 
        ...
        if sum(fIdx)==1 && sum(sIdx)==1 && size(fron(fIdx).fg1IntX, 1) == size(side(sIdx).fg1IntX, 1) % in case, there are both front and side video files for this trial
