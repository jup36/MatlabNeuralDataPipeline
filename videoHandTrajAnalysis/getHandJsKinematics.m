...
for t = 1:length(hTrjS)
    if ~isempty(hTrjS{t})
        ...
        hTrjSDistMed{1,t} = splitapply(medDist, hTrjS{t}, 1:size(hTrjS{t},2)); % point-by-point position relative to the global median
        hTrjSVelMed{1,t} = diff([hTrjSDistMed{1,t}(:,1) hTrjSDistMed{1,t}],1,2)./(4/1000); 
...
figure; plot(x,hTrjSDistMed{7}); 
xlabel('Time(ms)'); ylabel('Position relative to global median (mm)')
print(fullfile(filePath,'Figure',sprintf('handPosRelToGlobalMedian_tr#%d',7)),'-dpdf','-painters','-bestfit'); 

figure; plot(x,hTrjSVelMed{7}); 
xlabel('Time(ms)'); ylabel('Velocity relative to global median (mm/s)')
print(fullfile(filePath,'Figure',sprintf('handVelRelToGlobalMedian_tr#%d',7)),'-dpdf','-painters','-bestfit'); 
