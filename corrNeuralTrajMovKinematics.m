function corrNeuralTrajMovKinematics(filePath, fileName, saveNameTag)
%UNTITLED2 Summary of this function goes here
%   Detailed explanation goes here

filePath = 'Z:\parkj\NeuralData\ITphys\IT01_Ldms_M1_121317\Matfiles';
cd(filePath)
fileNameNeuralTraj = 'IT01Str_121317_pca_reward_5D_50msBin.mat';
fileNameBeh = 'BehVariables.mat';

% load/organize neural trjectories pca- or gpfa-based
neuralTrajFile = dir(fullfile(filePath,fileNameNeuralTraj));
load(neuralTrajFile.name,'pcaResult')
nTrj.trjMat = reshape([pcaResult.kern.seqTrain.xpost], pcaResult.p.Results.pcaDim, [], length(pcaResult.kern.seqTrain)); % neural trajectories in dim x timeBins x trials
nTrj.trialId = [pcaResult.kern.seqTrain.trialId]; % trial IDs for the neural trjectories
nTrj.relativeTimeBins = pcaResult.p.Results.timeRange(1):pcaResult.p.Results.pcaBinSize:pcaResult.p.Results.timeRange(2)-pcaResult.p.Results.pcaBinSize;
binsOfInt = nTrj.relativeTimeBins>=-2500 & nTrj.relativeTimeBins<-500; % the time bins of interest for the behavioral kinematics (e.g. when reaches are most likely to occur)
...
figure; imagescJP(rMaxPos',cmap,[-0.8 0.8]); pbaspect([1 1 1]); % plot the corr between nTrj and maxPos dim-by-dim
figure; imagescJP(rMaxVel',cmap,[-0.8 0.8]); pbaspect([1 1 1]); % plot the corr between nTrj and maxVel dim-by-dim
figure; imagescJP(rLick',cmap,[-0.8 0.8]); pbaspect([1 1 1]); % plot the corr between nTrj and lickCount dim-by-dim
...
sortMaxPos = sortrows([[bTrj.maxReachPos];[bTrj.trialId]]',-1); % sorted max reach positions
...
for f = 0:folds-1
    if f<folds-1
        nTrj.sortedFoldMaxPos{f+1} = nTrj.trjMat(:,:,sortMaxPos(f*trFolds+1:(f+1)*trFolds,2)); % take the trial-average within the current fold sorted by the max position
        nTrj.sortedFoldMaxVel{f+1} = nTrj.trjMat(:,:,sortMaxVel(f*trFolds+1:(f+1)*trFolds,2)); % take the trial-average within the current fold sorted by the max velocity
        nTrj.sortedFoldLickCnt{f+1} = nTrj.trjMat(:,:,sortLickCnt(f*trFolds+1:(f+1)*trFolds,2)); % take the trial-average within the current fold sorted by the lick counts
    elseif f==folds-1 % take all the remaining trials for the final fold
        nTrj.sortedFoldMaxPos{f+1} = nTrj.trjMat(:,:,sortMaxPos(f*trFolds+1:end,2)); % take the trial-average within the current fold sorted by the max position
...
eventMarkersRelative = [-1050 0];
eventMarkers = [1, arrayfun(@(x) find(x==nTrj.relativeTimeBins), eventMarkersRelative)];
...
nTrj.trAvgSortedFoldMaxPos = cellfun(@(x) nanmean(x(1:3,:,:),3), nTrj.sortedFoldMaxPos, 'UniformOutput', false); % use this for some computation across every cell entry
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function p = parse_input_corrNeuralTrajMovKinematics( filePath, fileName, saveNameTag, vargs ) % note that a nested function must use vargs not varargin when varargin was used for the main function
        ...
        addRequired(p,'saveNameTag');  % save name to save the outcome
        addRequired(p,'eventMarkers'); % specific events to be marked on each neural trajectory
