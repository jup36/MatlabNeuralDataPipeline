...
function run_reachPull_trialType_dPrime(filePath)
%% 0. Load data
cd(filePath)
...
if ~isempty(spkDir) || ~isempty(spkDir_Cg)
    ...
    if ~isempty(spkDir)  
        ...
        save(fullfile(filePath, 'Matfiles', strcat('glm_dPrime_',saveName)),'dPrmC','tV','p')
    end
    % Cg
    if ~isempty(spkDir_Cg)  
        for i_cell = 1:size(spkTimesCellCg,2)
            ...
            if spike_rate>=0.5
                ...
                % d' trial types
                dPrmC_Cg{i_cell,1} = dPrime_trialType(spike_rStart_conv, [tV.leloI(pStartI), tV.lehiI(pStartI), tV.riloI(pStartI), tV.rihiI(pStartI)], [-1 -1 1 1; -1 1 -1 1]);
        ...
        save(fullfile(filePath, 'Matfiles', strcat('glm_dPrime_Cg_',saveName)),'dPrmC_Cg','tV','p')
    end 
end
end
