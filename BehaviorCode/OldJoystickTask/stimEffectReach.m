...
stimR.pseudoReachPos = cell(length(fileList),1); % reach position detected near laser stim
stimR.reachPos1Dcm   = cell(length(fileList),1);
stimR.pseudoReachPos1Dcm = cell(length(fileList),1);
...
for f = 1:length(fileList) % increment files
    ...
    % get stimPos1 of pseudoReaches
    [ tempPseudoReachStart, ~, tempStmPos1 ] = getPseudoReachTimesForStimTrials( tempPositionData, tempTS.stmLaser ); % detect pseudo-reach trajectories around the time of laser stimulation deliveries. 
    
    stimR.pseudoReachPos{f,1} = tempStmPos1; % stim pseudo-reach amplitudes 

    tempDcmStmPos1 = zeros(size(tempStmPos1,1), size(tempStmPos1,2)/10); % preallocate tempDcmStmPos1 (decimated by a factor of 10)
    for t = 1:size(tempStmPos1,1)       % increment pseudoReaches
        tempDcmStmPos1(t,:) = decimate(tempStmPos1(t,:),10);
    end
    clearvars t
    stimR.pseudoReachPos1Dcm{f,1} = tempDcmStmPos1; % decimated stim pseudo-reach amplitudes
    
    % get the tempStmReachIdx by matching ts.stmLaser and ts.stmLaserReach 
    tempStmReachIdx = zeros(length(tempPseudoReachStart),1); % index for stmReaches (pseudoReaches actually satisfying reach criterion)
...
stimR.stmPos1Score = stimR.pseudoReachPos1Dcm{1,1}(:,1:100)*stimR.posCoeff; % project stmPos1 data to the principal components of the no-stim pos1 trajectories
...
stimR.velReaches    = diff([zeros(size(stimR.reachPos1Dcm,1),1)  stimR.reachPos1Dcm(:,1:100)],1,2); % velocity of reaches on decimated position data
stimR.maxVelReaches = max(stimR.velReaches,[],2); % max velocity of full reaches

stimR.velStimReaches = diff([zeros(size(stimR.pseudoReachPos1Dcm{1,1}(:,1:100),1),1)  stimR.pseudoReachPos1Dcm{1,1}(:,1:100)],1,2); % velocity of full reaches
stimR.maxVelStimReaches = max(stimR.velStimReaches,[],2); % max velocity of stim reaches

stimR.maxPosReaches = max(stimR.reachPos1Dcm(:,1:100),[],2); % max position of full reaches
stimR.maxPosStimReaches = max(stimR.pseudoReachPos1Dcm{1,1}(:,1:100),[],2); % max velocity of stim reaches
