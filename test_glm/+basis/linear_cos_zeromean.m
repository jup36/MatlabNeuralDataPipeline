function [bump, bin, cos_func] = linear_cos_zeromean(n_bump, range, dt, normalize)
if nargin < 1
    n_bump = 10;
end
...
%cos_func = @(x, peak) (cos(max(-pi, min(pi, (x - peak) * pi / (2 *gap))))+ 1) / 2; % cf this gets a range [0 1]
cos_func = @(x, peak) cos(max(-pi, min(pi, (x - peak) * pi / (2 * gap)))); % this gets a range [-1 1]
